...
test('inventory procurement and production use the shared specialist conversation body', async () => {
...
test('local module permission is mandatory and cross-domain expansion is permission bounded', async () => {
  const [inventoryRoute, procurementRoute, productionRoute, inventoryIntelligence] = await Promise.all([
...
test('shared runtime stays read-only canonical freshness-aware and module-first', async () => {
  ...
  assert.match(runtime, /No ejecutes compras, reservas, ajustes de stock, recepciones, aprobaciones, cambios de plan, cierres/);
