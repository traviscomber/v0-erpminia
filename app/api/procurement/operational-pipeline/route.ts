...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [{ data: requestLines, error: requestLinesError }, { data: orderLines, error: orderLinesError }] = await Promise.all([
      requestIds.length
        ? context.supabase.from('procurement_intake_request_lines').select('*').in('intake_request_id', requestIds)
        : Promise.resolve({ data: [], error: null }),
      orderIds.length
        ? context.supabase.from('procurement_operational_order_lines').select('*').in('order_id', orderIds)
        : Promise.resolve({ data: [], error: null }),
    ]);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'create_quotation') {
      const { data, error } = await context.supabase.rpc('create_intake_quotation', {
        p_intake_request_id: body.intakeRequestId,
    ...
    if (action === 'receive_order') {
      const { data, error } = await context.supabase.rpc('receive_operational_order', {
