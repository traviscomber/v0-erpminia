...
export async function GET(request: NextRequest) {
  ...
  if (area) { suppliers = suppliers.ilike('business_type', `%${area}%`); candidates = candidates.ilike('business_type', `%${area}%`); }
  ...
  if (sErr||cErr||eErr) return NextResponse.json({error:sErr?.message||cErr?.message||eErr?.message},{status:500});
...
export async function POST(request: NextRequest) {
  const auth=await requireAuth(request); if(!auth.authorized||!auth.user||!auth.organizationId)return auth.response;
  const body=await request.json().catch(()=>({})); if(!body.name)return NextResponse.json({error:'Nombre obligatorio'},{status:400});
  const supabase=getSupabaseServerClient();
  const {data,error}=await supabase.from('procurement_supplier_candidates').insert({organization_id:auth.organizationId,name:body.name,rut:body.rut||null,email:body.email||null,phone:body.phone||null,city:body.city||null,region:body.region||null,business_type:body.businessType||null,source_type:body.sourceType||'market_research',source_reference:body.sourceReference||null,requested_by:auth.user.id}).select('id').single();
  if(error)return NextResponse.json({error:error.message},{status:400}); return NextResponse.json(data,{status:201});
}

export async function PATCH(request: NextRequest) {
  const auth=await requireAuth(request); if(!auth.authorized||!auth.user)return auth.response;
  const body=await request.json().catch(()=>({})); const supabase=getSupabaseServerClient();
