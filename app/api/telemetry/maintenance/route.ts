...
export async function POST(request: NextRequest) {
  ...
  if (action === 'create_work_order') {
    ...
    const { data: workOrder, error } = await context.supabase.from('maintenance_work_orders').insert({ organization_id: context.organizationId, canonical_asset_id: event.canonical_asset_id, title: `Revisar condición de telemetría: ${sensorLabel}`, description: `Lectura registrada: ${event.observed_value} ${event.unit || sensor?.unit || ''}. Umbral configurado: ${event.threshold_value} ${event.unit || sensor?.unit || ''}. Fecha de lectura: ${event.event_at}. La OT solicita verificación de la condición; no constituye un diagnóstico automático.`, work_type: 'condition_based', status: 'open', priority, scheduled_date: new Date().toISOString().slice(0, 10), created_by: context.userId }).select('id, work_order_number, title').single();
