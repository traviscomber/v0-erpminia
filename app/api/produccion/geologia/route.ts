...
export async function GET(request: NextRequest) {
  ...
  const period=getDashboardPeriod(request);if(!period)return NextResponse.json({error:'Mes inválido'},{status:400});
  const drillingQuery=applyDatePeriod(context.supabase.from('production_drilling_source_reports').select('id,operation_date,hole_code_raw,mine_raw,sector_raw,drilled_meters,canonical_mine_source_id,canonical_mine_sector_id,canonical_drill_hole_id,reconciliation_status').eq('organization_id',context.organizationId),period,'operation_date');
  const recentDrillingQuery=applyDatePeriod(context.supabase.from('production_drilling_source_reports').select('id,operation_date,hole_code_raw,mine_raw,sector_raw,drilled_meters,reconciliation_status,canonical_mine_source_id,canonical_mine_sector_id,canonical_drill_hole_id').eq('organization_id',context.organizationId),period,'operation_date').order('operation_date',{ascending:false}).order('source_row',{ascending:false}).limit(200);
