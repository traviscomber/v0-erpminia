...
export async function GET(request: NextRequest) {
  ...
  const [summary, monthly, assets, schedules, plan, planLines, locationQueue] = await Promise.all([
    ...
    context.supabase.from('production_drill_hole_location_review_queue_v4').select('drill_hole_id,hole_code,resolution_state,report_count,last_report_date,source_site,candidate_mine_name,review_lane,review_priority,recommended_action,distinct_site_count,source_sites').eq('organization_id', context.organizationId).order('review_priority', { ascending: false }).order('last_report_date', { ascending: false }).limit(300),
  ]);
  ...
  return NextResponse.json({
    ...
    lineage: {
      ...
      note: 'ACTUAL y PLAN se mantienen separados. La ubicación sólo se promueve con evidencia verificable; alias tipográficos y conflictos de sitio se depuran por separado.',
    },
  });
}
