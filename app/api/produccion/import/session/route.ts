...
export async function POST(request: NextRequest) {
  ...
  if (!['mineral_transport', 'plant_metallurgy'].includes(mode)) {
    return NextResponse.json({ error: 'Modo de importación inválido' }, { status: 400 });
  }
  if (!sourceFile || !/^[a-f0-9]{64}$/.test(sourceFileSha256) || !templateVersion) {
    return NextResponse.json({ error: 'Archivo, SHA-256 y templateVersion son obligatorios' }, { status: 400 });
  }
  ...
  if (!batchId) {
    const { data: batch, error: batchError } = await context.supabase
      ...
      .insert({
        ...
        normalization_rule_version: templateVersion,
        notes: `Carga Excel validada en cliente contra ${templateVersion}.`,
  ...
  const { data: currentSession } = await context.supabase
  ...
  if (!sessionId) {
    const { data: session, error: sessionError } = await context.supabase
      ...
      .insert({
        ...
        status: existing?.status === 'imported' ? 'committed' : 'validated',
        validation_summary: {
          file: sourceFile,
  ...
  return NextResponse.json({
    batchId,
    sessionId,
    alreadyImported: existing?.status === 'imported',
    batchStatus: existing?.status || 'approved_for_import',
  });
}
