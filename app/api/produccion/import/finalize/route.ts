...
export async function POST(request: NextRequest) {
  ...
  if (!batchId || !Number.isInteger(expectedRows) || expectedRows < 0) {
    return NextResponse.json({ error: 'batchId y expectedRows válidos son obligatorios' }, { status: 400 });
  }
  ...
  if (!table) {
    return NextResponse.json({ error: 'Este endpoint de cierre solo soporta batches TM' }, { status: 400 });
  }
  ...
  if ((count || 0) !== expectedRows) {
    return NextResponse.json(
      {
        error: 'El batch no puede cerrarse porque el conteo persistido no coincide con la evidencia esperada',
        expectedRows,
