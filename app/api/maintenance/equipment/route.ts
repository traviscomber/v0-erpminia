...
export async function GET(request: NextRequest) {
  ...
  try {
    const { data, error } = await context.supabase
      ...
      .from('assets')
      .select(
        'id, asset_code, name, asset_type, category, manufacturer, model, serial_number, license_plate, cost_center_code, is_active, validation_status, validation_notes, source_file, source_sheet, source_row, updated_at',
      )
    ...
    if (error) {
      console.error('[maintenance/assets] Canonical asset fetch failed:', error);
      throw error;
    }

    const assets = (Array.isArray(data) ? data : []).map((asset: CanonicalAssetRow) => ({
      id: asset.id,
      asset_id: asset.id,
