...
export async function GET(request: NextRequest) {
  ...
  const probes = [
    ['drilling_maintenance_review_queue_v1', context.supabase.from('drilling_maintenance_review_queue_v1').select('review_id,canonical_asset_id,asset_code,asset_name,operation_date,review_reason,equipment_status_raw,machine_observations,review_status,has_linked_work_order').eq('organization_id', context.organizationId).limit(1)],
    ['drill_asset_operational_evidence_90d_v1', context.supabase.from('drill_asset_operational_evidence_90d_v1').select('canonical_asset_id,asset_code,asset_name,drilling_reports,out_of_service_reports,operational_with_observations_reports,operational_reports,equipment_without_crew_reports,power_outage_reports,water_shortage_reports,evidence_status').eq('organization_id', context.organizationId).limit(1)],
    ['preventive_maintenance_hour_status_v1', context.supabase.from('preventive_maintenance_hour_status_v1').select('schedule_id,canonical_asset_id,asset_code,asset_name,task_name,frequency_hours,effective_current_meter,due_meter,meter_evidence_source,meter_basis_conflict,hour_status,remaining_hours,generated_work_order_id').eq('organization_id', context.organizationId).limit(1)],
    ...
    ['maintenance_reliability_base_v1', context.supabase.from('maintenance_reliability_base_v1').select('work_order_id,canonical_asset_id,asset_code,asset_name,root_cause,root_cause_key,work_type,actual_duration_hours,down_time_hours,total_cost,closed_at').eq('organization_id', context.organizationId).limit(1)],
    ['work_order_close_readiness_v2', context.supabase.from('work_order_close_readiness_v2').select('work_order_id,work_order_number,canonical_asset_id,title,ready_to_close,next_action,open_procurement_orders,pending_parts,unmet_material_requirements,pending_external_services,open_labor_entries,external_cost_conflict,standard_plan_steps_pending').eq('organization_id', context.organizationId).limit(1)],
    ['maintenance_canonical_assets_v1', context.supabase.from('maintenance_canonical_assets_v1').select('id,asset_code,name,asset_type,category,manufacturer,model,cost_center_code,is_active,validation_status').eq('organization_id', context.organizationId).limit(1)],
  ] as const;
