...
async function loadCostSummary(context: Awaited<ReturnType<typeof getOrganizationContext>> & { ok: true }, workOrderId: string) {
  const { data, error } = await context.supabase.rpc('get_work_order_cost_summary', {
  ...
  return {
    ...
    external: Number(row?.external_cost || 0),
    total: Number(row?.total_cost || 0),
    pendingParts: Number(row?.pending_parts || 0),
    openLaborEntries: Number(row?.open_labor_entries || 0),
  };
}
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [asset, costSummary] = await Promise.all([
      loadCanonicalAsset(context, data.canonical_asset_id || null),
      loadCostSummary(context, id),
    ]);
    return NextResponse.json({ data: mapWorkOrder(data, asset, costSummary), canonical: true });
  } catch (error) {
    ...
    if (body.status === 'completed') {
      ...
      if (blockers.length > 0) {
        return NextResponse.json(
          { error: `No se puede completar la orden: ${blockers.join(' y ')}.`, blockers, costSummary },
          { status: 409 },
        );
      }
      ...
      const { data: closed, error: loadError } = await context.supabase
        .from('maintenance_work_orders')
        .select('*')
        .eq('id', id)
        .eq('organization_id', context.organizationId)
