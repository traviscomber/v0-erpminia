...
type DecisionCase = {
  ...
  impact:{baseline:string;expected:null;observed:null;status:'not_measured'};
};
...
type Response = {
  summary:{
    ...
    high:number;
    hypotheses:number;
    operationalEvidenceAssets:number;
    reliabilityClosuresEligibleForLearning:number;
    reliabilityClosuresExcludedAsSyntheticOrNonOperational:number;
  ...
  cases:DecisionCase[];
  semantics:string;
  decision_case_policy:{execution:string;persistence:string;impact:string};
...
export default function MaintenanceDecisionIntelligencePage(){
  ...
  return <div className="mx-auto w-full max-w-[1600px] space-y-6">
    ...
    <StatePanel tone="neutral" title="Frontera de confianza" description={`${data.semantics} La IA no aprueba, ejecuta ni cierra trabajo. Cada caso conserva evidencia, responsable y checkpoint humano.`} className="min-h-0 py-5"/>

    <Card className="shadow-none">
      <CardHeader className="border-b pb-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div><CardTitle className="text-lg">Cola de decisión</CardTitle><CardDescription>Prioridad operacional basada en evidencia canónica. Impacto permanece sin medir hasta registrar un resultado observado.</CardDescription></div>
          <Badge variant="outline">{cases.length} casos</Badge>
        </div>
      </CardHeader>
      <CardContent className="p-0">
        {cases.length===0?<StatePanel tone="neutral" title="Sin casos pendientes" description="Las fuentes actuales no muestran señales que requieran revisión adicional." className="min-h-64 border-0 bg-transparent"/>:<div className="divide-y">{cases.map((decisionCase,index)=><DecisionRow key={decisionCase.case_key} decisionCase={decisionCase} index={index}/>)}</div>}
      </CardContent>
    </Card>
  ...
  return <article className="grid gap-4 px-4 py-5 lg:grid-cols-[48px_minmax(0,1fr)_220px_44px] lg:items-start">
    ...
    <div className="min-w-0 space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <Badge variant={urgencyVariant[decisionCase.urgency]}>{urgencyLabel[decisionCase.urgency]}</Badge>
        <Badge variant="outline">{kindLabel[decisionCase.kind]||decisionCase.kind}</Badge>
        {decisionCase.asset_code?<Badge variant="outline">{decisionCase.asset_code}</Badge>:null}
        <span className="text-xs text-muted-foreground">{decisionCase.asset_name||'Activo no identificado'}</span>
      </div>
      <div>
        <h2 className="text-sm font-semibold leading-6">{decisionCase.canonical_fact}</h2>
        <p className="mt-1 text-sm leading-5 text-muted-foreground">{decisionCase.professional_interpretation}</p>
      </div>
    ...
    <div className="space-y-3 text-xs">
      <div className="flex items-center gap-2"><ShieldCheck className="h-4 w-4"/><div><p className="font-medium">Revisión humana</p><p className="text-muted-foreground">{decisionCase.accountable_role}</p></div></div>
      <div className="flex items-center gap-2">{hasGap?<CircleAlert className="h-4 w-4"/>:<CheckCircle2 className="h-4 w-4"/>}<div><p className="font-medium">{hasGap?'Evidencia incompleta':'Lista para revisar'}</p><p className="text-muted-foreground">{decisionCase.evidence_count} evidencia(s) · {decisionCase.missing_evidence.length} pendiente(s)</p></div></div>
      <div className="border-t pt-3"><p className="font-medium">Impacto aún no medido</p><p className="mt-1 text-muted-foreground">Línea base preservada; sin beneficio atribuido.</p></div>
    </div>
...
function Policy({title,text}:{title:string;text:string}){
  return <div className="border-t pt-4"><p className="text-sm font-medium">{title}</p><p className="mt-1 text-sm leading-5 text-muted-foreground">{text}</p></div>;
}
