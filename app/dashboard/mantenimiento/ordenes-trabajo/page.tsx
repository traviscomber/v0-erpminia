...
export default function WorkOrdersPage() {
  ...
  const scheduleItems = useMemo(() => operationalWorkOrders
    ...
    .slice(0, 7), [operationalWorkOrders]);
    ...
    <div className="space-y-6">
      <section className="flex flex-col gap-4 border-b border-border/70 pb-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm text-muted-foreground">{missingAssetOnly ? 'Cola Data Health: sólo OT operacionales de Motil que todavía no tienen un activo canónico asociado. El histórico importado no entra en esta cola.' : 'La operación nueva de Motil se mantiene separada del histórico importado. Los KPI, vencimientos y próximas intervenciones usan sólo OT operacionales; el histórico queda disponible como evidencia.'}</p>
        </div>
      ...
      {missingAssetOnly ? <Card className="border-destructive/30 bg-destructive/5 shadow-none"><CardContent className="flex items-start gap-3 p-4"><AlertCircle className="mt-0.5 h-5 w-5 text-destructive" /><div><p className="font-medium">Data Health · OT operacional sin activo canónico</p><p className="mt-1 text-sm text-muted-foreground">Esta vista no corrige automáticamente la asociación. El histórico importado queda fuera porque no representa trabajo nuevo pendiente en Motil.</p></div></CardContent></Card> : null}

      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[['Abiertas', open], ['En progreso', inProgress], ['Críticas', critical], ['Vencidas', overdue]].map(([label, value]) => (
          <Card key={String(label)} className="shadow-none"><CardContent className="p-4"><p className="text-xs text-muted-foreground">{String(label)}</p><p className="mt-1 text-2xl font-semibold">{Number(value)}</p><p className="mt-1 text-[11px] text-muted-foreground">Operación Motil</p></CardContent></Card>
        ))}
      </div>

      {!missingAssetOnly && historicalWorkOrders.length > 0 ? <Card className="shadow-none"><CardContent className="flex flex-col gap-2 p-4 text-sm sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">Histórico preservado</p><p className="text-muted-foreground">{historicalWorkOrders.length} OT importada(s) disponibles para consulta, sin contaminar la operación diaria.</p></div><Button variant="outline" size="sm" onClick={() => setScopeFilter('historical')}>Ver histórico</Button></CardContent></Card> : null}
      ...
      <Card className="shadow-none"><CardContent className="p-4"><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(240px,1fr)_170px_170px_170px_auto]">
        ...
        <Select value={missingAssetOnly ? 'operational' : scopeFilter} onValueChange={setScopeFilter} disabled={missingAssetOnly}><SelectTrigger><SelectValue placeholder="Origen" /></SelectTrigger><SelectContent><SelectItem value="operational">Operación Motil</SelectItem><SelectItem value="historical">Histórico</SelectItem><SelectItem value="all">Todo</SelectItem></SelectContent></Select>
        ...
        <Button variant="ghost" onClick={() => { setSearch(''); setStatusFilter('all'); setPriorityFilter('all'); setScopeFilter('operational'); }}>Limpiar</Button>
      </div></CardContent></Card>

      {!missingAssetOnly && scheduleItems.length > 0 ? <Card className="shadow-none"><CardHeader className="pb-3"><CardTitle className="text-base">Próximas intervenciones · Operación Motil</CardTitle></CardHeader><CardContent>{updatingScheduleId ? <p className="mb-3 text-sm text-muted-foreground">Actualizando orden...</p> : null}<MaintenanceSchedule schedules={scheduleItems} onMarkComplete={markScheduleComplete} /></CardContent></Card> : null}

      <Card className="shadow-none"><CardHeader className="pb-3"><CardTitle className="text-base">{missingAssetOnly ? 'OT operacionales pendientes de asociación canónica' : scopeFilter === 'historical' ? 'Histórico de órdenes' : scopeFilter === 'all' ? 'Todas las órdenes' : 'Órdenes operacionales Motil'}</CardTitle><p className="text-sm text-muted-foreground">{filteredOrders.length} de {workOrders.length} órdenes totales</p></CardHeader><CardContent>
        {isLoading ? <div className="space-y-2">{Array.from({ length: 5 }).map((_, index) => <div key={index} className="h-20 animate-pulse rounded-lg bg-muted" />)}</div> : error ? <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-8 text-center"><p className="font-medium text-destructive">No se pudieron cargar las órdenes</p><Button className="mt-4" variant="outline" onClick={() => void mutate()}>Reintentar</Button></div> : filteredOrders.length === 0 ? <div className="rounded-lg border border-dashed p-10 text-center text-sm text-muted-foreground">{missingAssetOnly ? 'No hay OT operacionales sin activo canónico para los filtros actuales.' : 'No hay órdenes que coincidan con los filtros.'}</div> : <div className="divide-y rounded-lg border">{filteredOrders.map((order) => { const historical = order.record_scope === 'historical'; return <div key={order.id} className="grid gap-4 p-4 transition-colors hover:bg-muted/30 lg:grid-cols-[minmax(0,1.5fr)_minmax(160px,.8fr)_minmax(150px,.7fr)_auto] lg:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-xs text-muted-foreground">{order.work_order_number || 'Sin folio'}</span><Badge variant="outline" className={getStatusClass(order.status)}>{getStatusLabel(order.status)}</Badge>{historical ? <Badge variant="secondary">Histórico</Badge> : null}{!historical && isOverdue(order) ? <Badge variant="destructive">Vencida</Badge> : null}{!historical && !order.asset_name ? <Badge variant="destructive">Sin activo</Badge> : null}</div><p className="mt-2 truncate font-medium">{order.title || 'Orden sin título'}</p><p className="mt-1 truncate text-sm text-muted-foreground">{order.asset_name || (historical ? 'Activo no normalizado en histórico' : 'Sin activo asociado')}</p></div><div><p className="text-xs text-muted-foreground">Tipo y prioridad</p><p className="mt-1 text-sm">{getWorkTypeLabel(order.work_type)} · {getPriorityLabel(order.priority)}</p></div><div><p className="text-xs text-muted-foreground">Responsable / fecha</p><p className="mt-1 text-sm">{order.assigned_to_name || 'Sin asignar'}</p><p className="text-xs text-muted-foreground">{order.scheduled_date ? new Date(order.scheduled_date).toLocaleDateString('es-CL') : 'Sin fecha'}</p></div><Button asChild variant="ghost" size="sm"><Link href={`/dashboard/mantenimiento/ordenes-trabajo/${order.id}`}><Eye className="mr-2 h-4 w-4" />Ver detalle</Link></Button></div>; })}</div>}
      </CardContent></Card>
    </div>
  );
}
