...
export default function WorkOrderDetailPage() {
  ...
  return <div className="space-y-6">
    <section className="flex flex-col gap-4 border-b border-border/70 pb-6 lg:flex-row lg:items-start lg:justify-between">
      <div><Button asChild variant="ghost" size="sm" className="-ml-3 mb-2"><Link href="/dashboard/mantenimiento/ordenes-trabajo"><ArrowLeft className="mr-2 h-4 w-4"/>Órdenes de trabajo</Link></Button><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-sm text-muted-foreground">{workOrder.work_order_number}</span><Badge variant="outline">{statusLabel(workOrder.status)}</Badge></div><h1 className="mt-2 text-3xl font-semibold tracking-tight">{workOrder.title || 'Orden de trabajo'}</h1><p className="mt-2 text-sm text-muted-foreground">{workOrder.asset_code || 'Sin código'} · {workOrder.asset_name || 'Sin equipo asociado'}</p></div>
      <div className="flex gap-2">{workOrder.status !== 'in_progress' && workOrder.status !== 'completed' ? <Button onClick={()=>void patchOrder({status:'in_progress'})} disabled={!canEdit}><PlayCircle className="mr-2 h-4 w-4"/>Iniciar trabajo</Button> : null}{workOrder.status === 'in_progress' ? <Button asChild><Link href={`/dashboard/mantenimiento/ordenes-trabajo/cierre?workOrderId=${id}`}><CheckCircle2 className="mr-2 h-4 w-4"/>Continuar cierre</Link></Button> : null}<DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" size="icon" aria-label="Más acciones"><MoreHorizontal className="h-4 w-4"/></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem disabled={!canEdit} onClick={()=>void patchOrder({status:'open'})}><RotateCcw className="mr-2 h-4 w-4"/>Reabrir orden</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>
    </section>

    <Card className="shadow-none"><CardHeader><CardTitle className="text-base">Resumen operativo</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6"><div><p className="text-xs text-muted-foreground">Estado</p><p className="mt-1 font-medium">{statusLabel(workOrder.status)}</p></div><div><p className="text-xs text-muted-foreground">Prioridad</p><p className="mt-1 font-medium">{priorityLabel(workOrder.priority)}</p></div><div><p className="text-xs text-muted-foreground">Tipo</p><p className="mt-1 font-medium">{typeLabel(workOrder.work_type)}</p></div><div><p className="text-xs text-muted-foreground">Responsable</p><p className="mt-1 font-medium">{workOrder.assigned_to_name || 'Sin asignar'}</p></div><div><p className="text-xs text-muted-foreground">Programada</p><p className="mt-1 font-medium">{workOrder.scheduled_date ? new Date(workOrder.scheduled_date).toLocaleDateString('es-CL') : 'Sin fecha'}</p></div><div><p className="text-xs text-muted-foreground">Lectura inicial</p><p className="mt-1 font-medium">{workOrder.meter_reading ? `${workOrder.meter_reading} ${workOrder.meter_unit || ''}` : 'Sin lectura'}</p></div></CardContent></Card>

    <Card className="shadow-none"><CardHeader><CardTitle className="text-base">Imputación financiera</CardTitle></CardHeader><CardContent><div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-end"><div className="space-y-2"><Label htmlFor="cost-center">Centro de costo</Label><select id="cost-center" value={workOrder.cost_center_id || ''} disabled={!canEdit} onChange={(event)=>void patchOrder({cost_center_id:event.target.value || null})} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"><option value="">Sin centro de costo</option>{costCenters.map((row:any)=><option key={row.id} value={row.id}>{row.code} · {row.name}</option>)}</select></div><div className="pb-2 text-sm"><span className={workOrder.cost_center_id ? 'text-foreground':'text-destructive'}>{workOrder.cost_center_id ? `Imputación lista: ${selectedCostCenter?.code || 'centro asignado'}` : 'Pendiente: Compras no podrá adjudicar una OC asociada a esta OT.'}</span></div></div></CardContent></Card>

    <Card className="shadow-none"><CardHeader><CardTitle className="text-base">Intervención</CardTitle></CardHeader><CardContent className="grid gap-5 lg:grid-cols-[1fr_320px]"><div><p className="text-sm leading-6">{workOrder.description || 'Sin descripción adicional.'}</p><div className="mt-5 grid gap-4 sm:grid-cols-2"><div><p className="text-xs text-muted-foreground">Causa principal</p><p className="mt-1 text-sm font-medium">{workOrder.root_cause || 'Pendiente en cierre progresivo'}</p></div><div><p className="text-xs text-muted-foreground">Acción preventiva</p><p className="mt-1 text-sm font-medium">{workOrder.preventive_actions || 'Pendiente en cierre progresivo'}</p></div></div></div><WorkOrderTimer workOrderId={id}/></CardContent></Card>
