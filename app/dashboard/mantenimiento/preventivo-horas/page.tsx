...
export default function PreventiveHoursPage() {
  ...
  return <div className="space-y-6">
    ...
    <Card className="shadow-none"><CardContent className="p-4 text-sm">{standardPlans.approved>0?<><p className="flex items-center gap-2 font-medium"><CheckCircle2 className="h-4 w-4"/>Procedimientos estándar disponibles</p><p className="mt-1 text-muted-foreground">{number(standardPlans.approved)} plan(es) aprobado(s) · {number(standardPlans.linkedSchedules)} pauta(s) preventiva(s) vinculada(s). Al generar una OT vinculada, Motil aplica automáticamente el procedimiento aprobado y sus requerimientos configurados.</p></>:<><p className="font-medium">Procedimiento estándar: evidencia pendiente</p><p className="mt-1 text-muted-foreground">Aún no existen planes estándar aprobados para esta organización. Define y aprueba el procedimiento antes de usarlo para enriquecer una OT.</p></>}</CardContent></Card>
