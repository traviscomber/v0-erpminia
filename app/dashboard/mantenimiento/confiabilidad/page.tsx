  ...
  return <div className="space-y-6">
    <section className="flex flex-col gap-4 border-b border-border/70 pb-6 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm text-muted-foreground">Aprende de cierres reales de OT. Costos, horas y causas se incorporan sólo cuando existe un snapshot de cierre auditado.</p>
      </div>
      <div className="flex gap-2"><Button asChild variant="outline"><Link href="/dashboard/mantenimiento/ordenes-trabajo/cierre">Cerrar OT</Link></Button><Button variant="outline" onClick={() => void mutate()}><RefreshCw className="mr-2 h-4 w-4"/>Actualizar</Button></div>
    </section>
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      {[
        ...
        ['Con causa recurrente', summary.assets_with_recurring_root_cause || 0],
        ['Horas reales', number(summary.total_actual_hours || 0)],
    ...
    <Card className="shadow-none">
      ...
      <CardContent className="p-0">
        {isLoading ? <div className="p-6 text-sm text-muted-foreground">Cargando evidencia auditada…</div> : assets.length === 0 ? <div className="p-8 text-center"><p className="font-medium">Todavía no hay cierres auditados</p><p className="mt-2 text-sm text-muted-foreground">Las 21 OT históricas sin snapshot no se usan para inferir recurrencia, costo ni intervalos. Esta vista empezará a aprender con los próximos cierres del flujo seguro.</p><Button className="mt-4" asChild variant="outline"><Link href="/dashboard/mantenimiento/ordenes-trabajo/cierre">Ir a cierre progresivo</Link></Button></div> : <div className="divide-y">{assets.map((row: any) => <div key={row.canonical_asset_id} className="p-4"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex flex-wrap gap-2"><Badge variant="outline">{row.audited_closures} cierre{Number(row.audited_closures) === 1 ? '' : 's'}</Badge>{row.has_recurring_root_cause ? <Badge variant="destructive">Causa recurrente</Badge> : <Badge variant="secondary">Sin recurrencia observada</Badge>}</div><p className="mt-3 font-medium">{row.asset_code ? `${row.asset_code} · ` : ''}{row.asset_name || 'Equipo sin nombre'}</p><div className="mt-2 grid gap-2 text-sm text-muted-foreground md:grid-cols-4"><span><Repeat2 className="mr-1 inline h-3.5 w-3.5"/>Intervalo: {interval(row.avg_days_between_audited_interventions)}</span><span><Clock3 className="mr-1 inline h-3.5 w-3.5"/>Detención: {number(row.total_downtime_hours)} h</span><span>Horas reales: {number(row.total_actual_hours)} h</span><span>Costo: {money(row.audited_total_cost)}</span></div></div><Button asChild size="sm" variant="outline"><Link href={`/dashboard/mantenimiento/equipos/${row.canonical_asset_id}`}><ExternalLink className="mr-2 h-4 w-4"/>Equipo</Link></Button></div></div>)}</div>}
      </CardContent>
    </Card>

    <Card className="shadow-none"><CardHeader><CardTitle className="text-lg">Causas raíz recurrentes</CardTitle></CardHeader><CardContent className="space-y-3">{recurringCauses.length === 0 ? <p className="text-sm text-muted-foreground">No hay causas raíz repetidas con evidencia auditada. Motil no infiere causas ausentes.</p> : recurringCauses.map((row: any) => <div key={`${row.canonical_asset_id}-${row.root_cause_key}`} className="rounded-lg border p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{row.asset_code ? `${row.asset_code} · ` : ''}{row.asset_name}</p><p className="mt-1 text-sm">{row.root_cause}</p></div><Badge variant="destructive">{row.occurrences} veces</Badge></div><p className="mt-2 text-xs text-muted-foreground">Costo auditado {money(row.audited_total_cost)} · Horas {number(row.total_actual_hours)} · Detención {number(row.total_downtime_hours)} h</p></div>)}</CardContent></Card>

    <Card className="shadow-none"><CardContent className="p-4 text-xs text-muted-foreground"><AlertTriangle className="mr-1 inline h-3.5 w-3.5"/>El intervalo mostrado es tiempo calendario entre intervenciones auditadas, no MTBF. Motil sólo podrá calcular MTBF cuando exista evidencia confiable de tiempo operativo o medición equivalente.</CardContent></Card>
  </div>;
}
