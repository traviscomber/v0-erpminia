...
function relativeDateLabel(daysUntil: number) {
  if (daysUntil < -1) return `Vencido hace ${Math.abs(daysUntil)} días`;
...
function statusClass(status: string) {
  ...
  if (['in_progress', 'partially_received'].includes(normalized)) {
...
export default function CalendarioPage() {
  ...
  const { data, error, isLoading, mutate, isValidating } = useSWR<CalendarResponse>(
    `/api/calendar/operational?days=${days}`,
    fetcher,
    {
  ...
  const metrics = [
    {
      ...
      icon: AlertTriangle,
      className: summary.overdue ? 'text-destructive' : 'text-muted-foreground',
    },
    {
      label: 'Hoy',
      value: summary.today,
      description: 'Acciones del día',
      icon: CalendarDays,
      className: 'text-foreground',
    },
    {
      label: 'Próximos 7 días',
      value: summary.next_7_days,
    ...
    },
    {
      label: 'En agenda',
      value: summary.total,
      description: `Horizonte de ${days} días`,
      icon: ListChecks,
      className: 'text-foreground',
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
            Fechas críticas de mantenimiento, cumplimiento y abastecimiento. Las modificaciones se realizan en el módulo de origen.
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <Card className="shadow-none">
        <CardContent className="grid gap-3 p-4 md:grid-cols-[minmax(0,1fr)_220px_180px]">
          <div className="relative">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
            <Input
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder="Buscar evento, responsable o referencia"
              className="pl-9"
            />
          </div>
      ...
      {data?.warnings?.length ? (
        <div className="flex gap-3 rounded-md border border-amber-500/30 bg-amber-500/10 p-4 text-sm">
          ...
          <div>
            <p className="font-medium">La agenda se cargó parcialmente.</p>
      ...
      {error ? (
        <Card className="border-destructive/40 shadow-none">
          <CardContent className="flex flex-col items-center gap-3 p-8 text-center">
            ...
            <div>
              <p className="font-medium">No fue posible cargar el calendario.</p>
      ...
      {!isLoading && !error && groups.length === 0 ? (
        <Card className="shadow-none">
          <CardContent className="flex flex-col items-center gap-3 p-10 text-center">
            ...
            <div>
              <p className="font-medium">No hay fechas para mostrar.</p>
              <p className="mt-1 text-sm text-muted-foreground">Ajusta los filtros o amplía el horizonte.</p>
            </div>
          </CardContent>
        </Card>
      ) : null}
                ...
                <div className="divide-y">
                  {dateItems.map((item) => {
                    ...
                    return (
                      <div
                        key={item.id}
                        className="grid gap-4 p-4 md:grid-cols-[44px_minmax(0,1fr)_auto] md:items-center"
                      >
                        ...
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Badge variant="outline" className={PRIORITY_CLASSES[item.priority]}>
                              {item.priority_label}
                            </Badge>
                          ...
                          <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                            ...
                            {item.owner ? (
                              <span className="inline-flex items-center gap-1">
                                <UserRound className="h-3.5 w-3.5" />{item.owner}
                              </span>
                            ) : null}
                            {item.location ? (
                              <span className="inline-flex items-center gap-1">
                        ...
                        <div className="flex items-center justify-between gap-4 md:flex-col md:items-end">
                          <p className={`text-xs font-medium ${item.overdue ? 'text-destructive' : 'text-muted-foreground'}`}>
                            {relativeDateLabel(item.days_until)}
                          </p>
      ...
      <Card className="border-dashed shadow-none">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">Alcance deliberadamente simple</CardTitle>
        </CardHeader>
        <CardContent className="text-sm text-muted-foreground">
          Esta vista no mueve fechas, no aprueba procesos y no crea recurrencias. Consolida lo que ya existe y lleva al usuario al módulo responsable para actuar.
        </CardContent>
      </Card>
    </div>
  );
