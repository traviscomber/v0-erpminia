...
export default function InspeccionesImportPage() {
  ...
  const downloadTemplate = () => {
    ...
    a.href = url;
    a.download = 'plantilla-inspecciones-sostenibilidad.csv';
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      if (!response.ok) {
        setResult({
          success: false,
          message: 'No se pudieron importar las inspecciones',
      ...
      setResult({
        success: true,
        message: payload.message || 'Inspecciones importadas correctamente',
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          <h1 className="text-3xl font-bold">Importar inspecciones</h1>
          <p className="mt-2 max-w-3xl text-muted-foreground">
            Carga inspecciones internas o externas desde Excel y deja el seguimiento operacional listo en un solo paso.
          </p>
        </div>
