...
export default function EPPImportPage() {
  ...
  const downloadTemplate = () => {
    const headers = ['CARGO_PUESTO', 'ELEMENTO_EPP', 'CANTIDAD_ELEMENTO', 'MARCA_MODELO', 'FICHA_TECNICA_URL', 'FRECUENCIA_REEMPLAZO', 'ACTIVO'];
    const rows = [
      ['Operador Mina', 'Casco con barbiquejo', '1', '3M', '', 'semestral', 'true'],
      ['Operador Mina', 'Lentes de seguridad', '1', 'Uvex', '', 'anual', 'true'],
    ];
    ...
    a.href = url;
    a.download = 'plantilla-epp-sostenibilidad.csv';
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      if (!response.ok) {
        setResult({
          success: false,
          message: 'No se pudo importar el EPP',
      ...
      setResult({
        success: true,
        message: payload.message || 'EPP importado correctamente',
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          <h1 className="text-3xl font-bold">Importar EPP</h1>
          <p className="mt-2 max-w-3xl text-muted-foreground">
            Sube una planilla para actualizar los elementos de protección personal por cargo sin reingresar los datos a mano.
          </p>
        </div>
