      ...
      <div className="space-y-5">
        ...
        {detail && !loading && <>
          ...
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">{[
            ['Existencia', Number(detail.summary.quantity_on_hand || 0).toLocaleString('es-CL'), Boxes],
            ['Disponible', Number(detail.summary.quantity_available || 0).toLocaleString('es-CL'), PackageSearch],
            ['Reservado', Number(detail.summary.quantity_reserved || 0).toLocaleString('es-CL'), ClipboardList],
            ['Valor inventario', money.format(Number(detail.summary.inventory_value || 0)), CircleDollarSign],
            ['Proveedores', detail.suppliers.length, ShoppingCart],
            ['Órdenes de trabajo', detail.workOrderUsage.length, Wrench],
          ].map(([label, value, Icon]) => <article key={String(label)} className="rounded-lg border bg-card p-4"><div className="flex justify-between text-muted-foreground"><span className="text-xs">{String(label)}</span><Icon className="h-4 w-4"/></div><p className="mt-2 text-xl font-semibold">{String(value)}</p></article>)}</section>
