...
export default function BodegaPage() {
  ...
  return <div className="space-y-6">
    ...
    <section aria-label="Resumen de inventario" className="grid gap-px overflow-hidden rounded-lg border bg-border sm:grid-cols-2 xl:grid-cols-4">{[
      ...
      ['Valor del inventario', valuationTrusted ? money(overview.total_stock_value) : 'En conciliación'],
      ['Por reponer', number(overview.reorder_products)],
      ['Conteos pendientes', number(overview.count_overdue_products)],
    ].map(([label, value]) => <div key={label} className="bg-card px-5 py-4"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 text-2xl font-semibold tracking-tight">{isLoading ? '—' : value}</p>{label === 'Valor del inventario' && !valuationTrusted && !isLoading ? <p className="mt-1 text-xs text-muted-foreground">Petróleo excluye una valorización confiable hasta conciliar fuentes.</p> : null}</div>)}</section>
    ...
    <section className="space-y-4">
      ...
      {!isLoading && positions.length > 0 ? <div className="overflow-hidden rounded-lg border bg-card"><div className="hidden grid-cols-[140px_1fr_120px_120px_130px_140px_36px] gap-4 border-b bg-muted/40 px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground lg:grid"><span>Código</span><span>Producto</span><span>Disponible</span><span>Reservado</span><span>Valor</span><span>Estado</span><span /></div>{positions.map((row) => {
        const conflictedDiesel = row.product_code === 'Combustible001' && Boolean(diesel?.hasConflict);
        return <Link key={row.stock_id} href={`/dashboard/bodega/productos/${row.product_id}`} className="group grid gap-2 border-b px-4 py-4 transition-colors last:border-0 hover:bg-muted/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring lg:grid-cols-[140px_1fr_120px_120px_130px_140px_36px] lg:items-center lg:gap-4"><p className="font-mono text-sm">{row.product_code}</p><div><p className="font-medium group-hover:text-primary">{row.product_name}</p><p className="text-xs text-muted-foreground">{row.family || 'Sin familia'} · {row.unit || 'unidad'}</p></div><p className="text-sm"><span className="text-muted-foreground lg:hidden">Disponible: </span>{conflictedDiesel ? 'En conciliación' : number(row.quantity_available)}</p><p className="text-sm"><span className="text-muted-foreground lg:hidden">Reservado: </span>{number(row.quantity_reserved)}</p><p className="text-sm font-medium">{conflictedDiesel ? '—' : money(row.stock_value)}</p>{conflictedDiesel ? <Badge variant="destructive">Revisar fuente</Badge> : <Badge variant={statusVariant(row.stock_status)}>{statusLabels[row.stock_status] || 'Revisar'}</Badge>}<ArrowRight className="hidden h-4 w-4 text-muted-foreground transition-transform group-hover:translate-x-1 lg:block" /></Link>;
      })}</div> : null}
    </section>
  </div>;
}
