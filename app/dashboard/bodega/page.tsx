...
const statusLabels: Record<string, string> = {
  ...
  negative: 'Revisar saldo',
  expired: 'Vencido',
  expiring: 'Próximo a vencer',
};

function statusVariant(status: string) {
  if (['out_of_stock', 'negative', 'expired'].includes(status)) return 'destructive' as const;
  if (['reorder', 'expiring'].includes(status)) return 'secondary' as const;
...
export default function BodegaPage() {
  ...
  const endpoint = useMemo(() => {
    ...
    return `/api/inventory/intelligence?${params.toString()}`;
  }, [query, status]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader>
        <PageHeaderContent>
          <PageHeaderEyebrow>Abastecimiento · Inventario</PageHeaderEyebrow>
          <PageHeaderTitle>Inventario</PageHeaderTitle>
          <PageHeaderDescription>
            Existencias, reservas, reposición y valor de stock en una sola vista.
          </PageHeaderDescription>
        </PageHeaderContent>
        <PageHeaderActions>
          <Button asChild variant="outline">
            <Link href="/dashboard/bodega/documentos"><FileText className="h-4 w-4" />Documentos</Link>
          </Button>
          <Button asChild>
            <Link href="/dashboard/compras/importar-existencias"><Upload className="h-4 w-4" />Importar existencias</Link>
          </Button>
        </PageHeaderActions>
      </PageHeader>
      ...
      <section aria-label="Resumen de inventario" className="grid gap-px overflow-hidden rounded-lg border bg-border sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          ['Productos por reponer', number(overview.reorder_products)],
          ['Conteos pendientes', number(overview.count_overdue_products)],
        ].map(([label, value]) => (
          <div key={label} className="bg-card px-5 py-4">
            <p className="text-xs text-muted-foreground">{label}</p>
            <p className="mt-1 text-2xl font-semibold tracking-tight">{isLoading ? '—' : value}</p>
          </div>
        ))}
      </section>
      ...
      <section className="space-y-4">
        <div className="flex flex-col gap-3 rounded-lg border bg-card p-3 lg:flex-row lg:items-center">
          ...
          <Select value={status} onValueChange={setStatus}>
            ...
            <SelectContent>
              ...
              <SelectItem value="healthy">Disponible</SelectItem>
              <SelectItem value="reorder">Reponer</SelectItem>
              <SelectItem value="out_of_stock">Sin stock</SelectItem>
              <SelectItem value="negative">Revisar saldo</SelectItem>
        ...
        {!isLoading && positions.length > 0 ? (
          <div className="overflow-hidden rounded-lg border bg-card">
            <div className="hidden grid-cols-[140px_1fr_120px_120px_130px_140px_36px] gap-4 border-b bg-muted/40 px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground lg:grid">
            ...
            {positions.map((row) => (
              <Link
                ...
                className="group grid gap-2 border-b px-4 py-4 transition-colors last:border-0 hover:bg-muted/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring lg:grid-cols-[140px_1fr_120px_120px_130px_140px_36px] lg:items-center lg:gap-4"
              >
                <p className="font-mono text-sm">{row.product_code}</p>
                <div>
                  <p className="font-medium group-hover:text-primary">{row.product_name}</p>
                  <p className="text-xs text-muted-foreground">{row.family || 'Sin familia'} · {row.unit || 'unidad'}</p>
                </div>
                <p className="text-sm"><span className="text-muted-foreground lg:hidden">Disponible: </span>{number(row.quantity_available)}</p>
                <p className="text-sm"><span className="text-muted-foreground lg:hidden">Reservado: </span>{number(row.quantity_reserved)}</p>
                <p className="text-sm font-medium">{money(row.stock_value)}</p>
                <Badge variant={statusVariant(row.stock_status)}>{statusLabels[row.stock_status] || 'Revisar'}</Badge>
