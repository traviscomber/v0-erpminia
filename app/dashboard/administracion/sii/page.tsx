      ...
      <section className="rounded-xl border bg-card p-6 shadow-sm">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-muted-foreground">
            El archivo se procesa sólo en el servidor. Motil valida titular, vigencia y llave privada, normaliza el material de firma y lo guarda cifrado en Supabase Vault. La contraseña no se vuelve a mostrar.
          </p>
        </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <button
            type="button"
            onClick={() => {
              setUploadMode('pkcs12');
              setCertificate(null);
              setPrivateKey(null);
              setPassphrase('');
            }}
            className={`rounded-lg border p-4 text-left ${uploadMode === 'pkcs12' ? 'border-foreground bg-muted/40' : 'bg-background'}`}
          >
            <span className="text-sm font-semibold">PFX / P12 · recomendado</span>
            <span className="mt-1 block text-xs text-muted-foreground">El formato habitual entregado por proveedores de certificado digital en Chile.</span>
          </button>
          <button
            type="button"
            onClick={() => {
            ...
            className={`rounded-lg border p-4 text-left ${uploadMode === 'pem' ? 'border-foreground bg-muted/40' : 'bg-background'}`}
          >
            <span className="text-sm font-semibold">PEM · avanzado</span>
            <span className="mt-1 block text-xs text-muted-foreground">Para TI: certificado público y llave privada por separado.</span>
          </button>
            ...
            <>
              <label className="space-y-2 text-sm md:col-span-2">
                ...
                <input
                  ...
                  onChange={(event) => setPkcs12(event.target.files?.[0] || null)}
                  className="block w-full rounded-md border bg-background p-2 text-sm"
                />
                <span className="block text-xs text-muted-foreground">Acepta .pfx y .p12 directamente; no necesitas convertirlo a PEM.</span>
              </label>
              <label className="space-y-2 text-sm md:col-span-2">
                ...
                <input
                  ...
                  className="h-10 w-full rounded-md border bg-background px-3 outline-none focus:ring-2 focus:ring-ring"
                />
                <span className="block text-xs text-muted-foreground">Se usa sólo durante la importación y no se conserva después de extraer la llave al vault cifrado.</span>
              </label>
            </>
          ) : (
            <>
              ...
              <label className="space-y-2 text-sm md:col-span-2">
                ...
                <input
                  ...
                  value={passphrase}
                  onChange={(event) => setPassphrase(event.target.value)}
                  placeholder="Sólo si la llave PEM está cifrada"
                  className="h-10 w-full rounded-md border bg-background px-3 outline-none focus:ring-2 focus:ring-ring"
                />
              </label>
            </>
        ...
        <button
          ...
          disabled={savingCertificate || !companyRut || !certificateReadyToSave}
