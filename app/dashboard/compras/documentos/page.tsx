...
export default function ComprasDocumentosPage() {
  ...
  const loadDocuments = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/documents/listmodule=Compras&category=documentos', {
        credentials: 'include',
      });
      ...
      if (Array.isArray(data)) {
        ...
        setStats({
          ...
          rechazados: data.filter((d: Document) => d.status === 'rechazado').length,
        });
      }
    } catch (error) {
        ...
        <TabsContent value="upload" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Sube órdenes de compra, contratos, facturas y documentos de proveedores
              </CardDescription>
            </CardHeader>
            <CardContent>
              <DocumentUpload 
