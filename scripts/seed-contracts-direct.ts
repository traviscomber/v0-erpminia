...
async function seedContracts() {
  try {
    // First, get or create a contractor
    ...
    console.log(`✓ Successfully seeded ${insertedContracts?.length || 0} contracts`);
    console.log(
      'Contracts:',
      insertedContracts?.map((c) => ({ id: c.id, number: c.contract_number, title: c.title }))
    );
