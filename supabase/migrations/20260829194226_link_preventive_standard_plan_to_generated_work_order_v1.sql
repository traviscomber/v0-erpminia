...
begin
  ...
  where a.organization_id = s.organization_id
    and a.preventive_schedule_id = s.id
    and a.status = 'active'
    and p.status = 'approved'
  order by a.applied_at desc, a.created_at desc, a.id desc
  limit 1;
  ...
  if s.generated_work_order_id is not null
    ...
    if v_plan_id is not null
       and not exists (
         ...
         from public.maintenance_standard_job_plan_applications a
         where a.organization_id = s.organization_id
           and a.work_order_id = s.generated_work_order_id
           and a.status = 'active'
       ) then
      insert into public.maintenance_standard_job_plan_applications(
        organization_id,
        plan_id,
        work_order_id,
        status,
  ...
  insert into public.work_order_events(
    ...
    jsonb_build_object('schedule_id', p_schedule_id, 'standard_plan_id', v_plan_id)
  );
...
revoke all on function public.create_work_order_from_schedule(uuid, uuid) from public, anon, authenticated;
grant execute on function public.create_work_order_from_schedule(uuid, uuid) to service_role;
