...
with cost_by_finance_asset as (
  ...
  left join canonical.asset_costs c
    on c.organization_id=r.organization_id and c.asset_code=r.finance_asset_code
...
select p.organization_id,
       ...
       d.drilled_meters,
       d.last_drilling_at,
       case when c.recognized_cost_clp is not null and d.drilled_meters is not null and d.drilled_meters>0 then c.recognized_cost_clp/d.drilled_meters else null::numeric end as lifetime_cost_clp_per_meter_preview,
       case when a.id is not null then 'canonicalized'::text else 'review_required'::text end as identity_status,
