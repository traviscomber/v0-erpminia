...
begin
  ...
  insert into public.procurement_operational_orders(organization_id,order_number,intake_request_id,awarded_quotation_id,supplier_id,work_order_id,canonical_asset_id,total_amount,expected_delivery_date,issued_by)
  values(v_quote.organization_id,v_order_number,v_req.id,v_quote.id,v_quote.supplier_id,v_req.work_order_id,v_req.canonical_asset_id,v_quote.total_amount,current_date+coalesce(v_quote.lead_time_days,0),public.current_application_user_id()) returning id into v_order_id;
  insert into public.procurement_operational_order_lines(organization_id,order_id,intake_line_id,canonical_product_id,product_code,description,unit,quantity_ordered,unit_cost)
  select ql.organization_id,v_order_id,ql.intake_line_id,ql.canonical_product_id,irl.product_code,irl.description,irl.unit,ql.quantity,ql.unit_cost
  from public.procurement_intake_quotation_lines ql join public.procurement_intake_request_lines irl on irl.id=ql.intake_line_id where ql.quotation_id=v_quote.id;
  update public.procurement_intake_quotations set status=case when id=v_quote.id then 'awarded' else 'rejected' end,updated_at=now() where intake_request_id=v_req.id and status='received';
  update public.procurement_intake_requests set status='ordered',updated_at=now() where id=v_req.id;
  ...
  values(v_req.organization_id,v_req.work_order_id,v_req.canonical_asset_id,'purchase_order_issued',public.current_application_user_id(),'procurement_operational_orders',v_order_id::text,'OC operativa emitida',jsonb_build_object('order_id',v_order_id,'order_number',v_order_number,'total_amount',v_quote.total_amount,'cost_center_id',v_work_order.cost_center_id,'cost_center_code',v_cost_center.code));
  return v_order_id;
end
$function$;
