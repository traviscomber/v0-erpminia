...
with cost as (
  ...
  select
    organization_id,
    canonical_asset_id,
    max(operating_date) as last_availability_date,
    count(*) filter (where operating_date >= current_date - 29) as availability_days_30d,
    sum(scheduled_minutes) filter (where operating_date >= current_date - 29) as scheduled_minutes_30d,
    sum(planned_downtime_minutes + unplanned_downtime_minutes) filter (where operating_date >= current_date - 29) as downtime_minutes_30d,
    case
      when sum(scheduled_minutes) filter (where operating_date >= current_date - 29) > 0
      then round(
        100.0 * (
          sum(scheduled_minutes) filter (where operating_date >= current_date - 29)
          - sum(planned_downtime_minutes + unplanned_downtime_minutes) filter (where operating_date >= current_date - 29)
        )::numeric
        / nullif(sum(scheduled_minutes) filter (where operating_date >= current_date - 29), 0),
        2
      )
      else null
    end as availability_pct_30d
  from public.asset_availability_daily_v1
  group by organization_id, canonical_asset_id
)
select
  a.organization_id,
  ...
  case
    when av.availability_pct_30d is not null then 'observed_30d'
    when coalesce(s.sensor_reading_count, 0) > 0 then 'telemetry_without_schedule'
    when coalesce(s.work_order_count, 0) > 0 and s.recorded_downtime_hours is not null then 'downtime_without_schedule'
    when coalesce(s.work_order_count, 0) > 0 then 'work_orders_without_downtime'
    else 'insufficient_evidence'
  end as availability_evidence_status,
  av.availability_pct_30d as availability_pct,
  av.last_availability_date,
  av.availability_days_30d,
  av.scheduled_minutes_30d,
  av.downtime_minutes_30d
...
left join availability av
  on av.organization_id = a.organization_id
 and av.canonical_asset_id = a.id;
