...
begin
  ...
  select re.evidence_status,re.runtime_reading_id into v_runtime_evidence_status,v_runtime_reading_id
  ...
  select * into v_cost from public.work_order_final_cost_v1
  where organization_id=v_wo.organization_id and work_order_id=p_work_order_id;
  ...
  from public.work_order_closure_cost_snapshots where work_order_id=p_work_order_id;
  ...
  set status='completed',completion_date=now(),closed_at=now(),closed_by=v_actor,updated_at=now()
  where id=p_work_order_id;
  update public.work_order_supply_needs set status='covered',updated_at=now()
  where work_order_id=p_work_order_id and status <> 'cancelled';
  update public.procurement_intake_requests set status='closed',updated_at=now()
  where work_order_id=p_work_order_id and status not in ('cancelled','closed');
  update public.procurement_operational_orders set status='closed',updated_at=now()
  where work_order_id=p_work_order_id and status='received';
  ...
  insert into public.work_order_events(
    ...
    jsonb_build_object(
      ...
      'runtime_evidence_status',v_runtime_evidence_status,'runtime_reading_id',v_runtime_reading_id,
...
comment on function public.close_work_order_safely(uuid) is
'Closes a maintenance work order with cost, plan and runtime-evidence audit. Runtime evidence is captured whenever present; corrective work orders require it.';
