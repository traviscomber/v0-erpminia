...
with candidate_overlap as (
  ...
  select u.organization_id, u.drill_hole_id, u.from_m, u.to_m, u.structural_features, u.source_rows
  from public.production_geology_contiguous_units_v1 u
  join candidate_overlap c on c.organization_id=u.organization_id and c.drill_hole_id=u.drill_hole_id
  ...
  select m.organization_id, m.drill_hole_id, m.hole_code,
    round(sum(greatest(0, least(m.to_m,s.to_m)-greatest(m.from_m,s.from_m)))::numeric,2) as overlap_m,
    min(greatest(m.from_m,s.from_m)) as first_overlap_from_m,
    max(least(m.to_m,s.to_m)) as last_overlap_to_m,
  ...
  left join lateral unnest(coalesce(m.source_rows,'{}'::integer[]) || coalesce(s.source_rows,'{}'::integer[])) x(row_no) on true
  group by m.organization_id,m.drill_hole_id,m.hole_code
