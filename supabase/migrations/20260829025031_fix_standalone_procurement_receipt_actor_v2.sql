...
language plpgsql
security definer
set search_path to 'public', 'canonical', 'auth', 'pg_temp'
as $function$
declare
  ...
  v_old_cost numeric;
  v_new_cost numeric;
  v_application_actor uuid := public.current_application_user_id();
  v_auth_actor uuid;
begin
  ...
  if v_order.organization_id not in (select organization_id from public.user_roles where user_id=v_application_actor) then raise exception 'Sin permisos'; end if;
  if v_order.status not in ('issued','partially_received') then raise exception 'OC no recepcionable'; end if;

  select l.auth_user_id into v_auth_actor
  from public.auth_profile_identity_links l
  where l.profile_id=v_application_actor;
  if v_auth_actor is null and exists(select 1 from auth.users u where u.id=v_application_actor) then
    v_auth_actor := v_application_actor;
  end if;
  ...
  values(v_order.organization_id,v_receipt_number,v_order.id,v_application_actor,p_notes) returning id into v_receipt_id;
  for v_line in select * from jsonb_array_elements(p_lines) loop
    ...
    if v_accepted>0 then
      ...
      values(v_order.organization_id,v_stock_id,'receipt',v_accepted::integer,'operational_receipt',v_receipt_id,v_auth_actor,'Recepción de OC operativa',p_notes,v_order.work_order_id,v_order.canonical_asset_id,v_ol.canonical_product_id,v_ol.unit_cost,v_accepted*v_ol.unit_cost,now());
    end if;
  end loop;
  ...
  if v_order.work_order_id is not null then
    perform public.recalculate_work_order_material_coverage(v_order.work_order_id, v_application_actor);
    ...
    values(v_order.organization_id,v_order.work_order_id,v_order.canonical_asset_id,'purchase_received',v_application_actor,'procurement_operational_receipts',v_receipt_id::text,'Materiales recibidos para la OT',jsonb_build_object('receipt_id',v_receipt_id,'receipt_number',v_receipt_number));
  end if;
  return v_receipt_id;
end
$function$;
