...
-- Preserves RAW source rows and applies only deterministic derived corrections.
...
with normalized as (
  select
    ...
    case when r.source_row = 3301 then 7::numeric else r.drilled_meters end as drilled_meters,
    case
    ...
    case when r.source_row = 88 then r.operation_date + interval '3 days' else r.operation_date end as chronology_order_at
