...
export function AssetImport({ onSuccess }: AssetImportProps) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch('/api/maintenance/import-assets', {
        method: 'POST',
        credentials: 'include',
        body: formData,
      });
      ...
      if (!response.ok) {
        setResult({
          success: false,
          message: 'No se pudieron importar los activos',
      ...
      setResult({
        success: true,
        message: payload.message || 'Activos importados correctamente',
    ...
    <Card className="border-[var(--secondary)]/25 bg-[var(--secondary)]/5">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          ...
          Importar vehiculos y activos desde Excel
        </CardTitle>
      </CardHeader>
        ...
        <Alert>
          ...
          <AlertDescription>
            ...
            <div className="rounded bg-muted p-2 font-mono text-sm">
              CODIGO | NOMBRE | TIPO | UBICACION | ESTADO | MODELO | SERIE
            </div>
          </AlertDescription>
        </Alert>
