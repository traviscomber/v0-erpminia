...
const groupOrder = ['General', 'Lean', 'Operaciones', 'Mantenimiento', 'Abastecimiento', 'Sostenibilidad y HSE', 'Administración y control', 'Ayuda'];

const groupDescriptions: Record<string, string> = {
  General: 'Visión ejecutiva y alertas',
  Lean: 'Control diario y mejora continua',
...
const itemModuleKey: Record<string, string> = {
  Alertas: 'core_alertas', 'Centros de Costos': 'core_centros_costos',
  Producción: 'prod_operaciones', 'Telemetría de Sensores': 'prod_telemetria',
  Mantenimiento: 'mant_operaciones', 'Órdenes de Trabajo': 'mant_operaciones',
  'Planificación Preventiva': 'mant_operaciones', 'Bitácora de Mantenimiento': 'mant_operaciones',
  Equipos: 'mant_operaciones', 'Vehículos y Traslados': 'mant_operaciones',
  'Personal Mantención': 'mant_recursos', Combustible: 'mant_recursos',
  'Indicadores de Mantención': 'mant_gerencial', 'Documentos Mantenimiento': 'mant_documentos',
  'Bodega e Inventario': 'bodega_inventario', 'Documentos Bodega': 'bodega_documentos',
  'Importar Existencias': 'bodega_inventario', 'Compras y OCs': 'fin_compras',
  'Documentos Compras': 'fin_compras', 'Finanzas y Presupuesto': 'fin_finanzas',
  Proveedores: 'fin_finanzas', 'Documentos Finanzas': 'fin_finanzas',
  'Reportes y Análisis': 'fin_reportes', 'Tablero Sostenibilidad': 'sos_tablero',
  'Prevención de Riesgos': 'hse_riesgos', 'Documentos Prevención': 'hse_documentacion',
  Capacitaciones: 'hse_capacitaciones', 'Artículos EPP': 'hse_epp',
  Inspecciones: 'sos_calendario', Calendario: 'sos_calendario',
  'Medio Ambiente': 'sos_medio_ambiente', Comunidades: 'sos_comunidades',
  'Flujo Documental': 'sos_documentos', 'Módulo Legal': 'legal_modulo',
  'Documentos Legales': 'legal_modulo', 'Gestión Documental': 'legal_contratos',
  Contratos: 'legal_contratos', 'Empresas Contratistas (EECC)': 'legal_eecc',
};
...
const improvementRoles = ['superadmin', 'admin', 'manager', 'supervisor', 'Operaciones-Supervisor', 'Sostenibilidad-Supervisor', 'HSE-Supervisor', 'jefe_mantencion'];
...
const rolePermissions: Record<string, string[]> = {
  ...
  'Centro Lean': operationalRoles, 'Daily Management': operationalRoles, Andon: operationalRoles,
  'Kanban operacional': operationalRoles, 'Kaizen + PDCA': improvementRoles,
  ...
  'Módulo Legal': ['superadmin', 'admin', 'manager'], 'Gestión Documental': ['superadmin', 'admin', 'manager'],
  Contratos: ['superadmin', 'admin', 'manager'], 'Empresas Contratistas (EECC)': ['superadmin', 'admin', 'manager'],
  'Documentos Legales': ['superadmin', 'admin', 'manager'], 'Roles y Cargos': ['superadmin', 'admin'],
...
const menuItems: MenuItem[] = [
  ...
  { label: 'Centro Lean', href: '/dashboard/lean', icon: Activity, group: 'Lean', section: 'Control' },
  { label: 'Daily Management', href: '/dashboard/daily-management', icon: CalendarDays, group: 'Lean', section: 'Control' },
  { label: 'Andon', href: '/dashboard/andon', icon: AlertTriangle, group: 'Lean', section: 'Respuesta' },
  { label: 'Kanban operacional', href: '/dashboard/kanban', icon: Columns3, group: 'Lean', section: 'Flujo' },
  { label: 'Kaizen + PDCA', href: '/dashboard/kaizen', icon: Lightbulb, group: 'Lean', section: 'Mejora' },
...
function getActiveGroup(pathname: string, items: MenuItem[]) {
  ...
  if (pathname.startsWith('/dashboard/lean') || pathname.startsWith('/dashboard/daily-management') || pathname.startsWith('/dashboard/andon') || pathname.startsWith('/dashboard/kanban') || pathname.startsWith('/dashboard/kaizen')) return 'Lean';
  if (pathname.startsWith('/dashboard/mantenimiento/')) return 'Mantenimiento';
  if (pathname.startsWith('/dashboard/sostenibilidad/')) return 'Sostenibilidad y HSE';
  if (pathname.startsWith('/dashboard/bodega/') || pathname.startsWith('/dashboard/compras/') || pathname.startsWith('/dashboard/finanzas/')) return 'Abastecimiento';
  if (pathname.startsWith('/dashboard/legal/') || pathname.startsWith('/dashboard/documentos-gestion/') || pathname.startsWith('/dashboard/admin/')) return 'Administración y control';
  return undefined;
}
...
export function Sidebar() {
  ...
  const handleNavigation = (href: string) => { router.push(href); setIsOpen(false) };

  return <>
    <Button variant="outline" size="icon" aria-label={isOpen ? 'Cerrar navegación' : 'Abrir navegación'} className="fixed left-4 top-4 z-50 bg-background shadow-sm lg:hidden" onClick={() => setIsOpen((current) => !current)}>
      {isOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
    </Button>
    <aside className={cn('fixed inset-y-0 left-0 z-40 flex h-screen w-[292px] flex-col border-r border-sidebar-border bg-sidebar transition-transform duration-200 lg:static lg:translate-x-0', isOpen ? 'translate-x-0' : '-translate-x-full')}>
      <div className="border-b border-sidebar-border px-5 py-5">
        <Link href="/dashboard" className="flex items-center gap-3 rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-orange-500/10 text-orange-600 dark:text-orange-400"><Activity className="h-5 w-5" /></div>
          <div className="min-w-0"><p className="truncate text-sm font-semibold tracking-tight">MOTIIL</p><p className="truncate text-xs text-muted-foreground">Gestión operacional</p></div>
        </Link>
      </div>
      <nav className="flex-1 overflow-y-auto px-3 py-4" aria-label="Navegación principal">
        <div className="space-y-2">
          {groupOrder.map((group) => {
            ...
            return <section key={group} className="rounded-xl border border-transparent data-[active=true]:border-border/60 data-[active=true]:bg-muted/20" data-active={activeGroup === group}>
              <button type="button" onClick={() => toggleGroup(group)} aria-expanded={isExpanded} className="flex w-full items-center justify-between gap-3 rounded-xl px-3 py-2.5 text-left transition-colors hover:bg-muted/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                <span className="min-w-0"><span className="block truncate text-sm font-semibold">{group}</span><span className="block truncate text-[11px] text-muted-foreground">{groupDescriptions[group]}</span></span>
                <ChevronDown className={cn('h-4 w-4 shrink-0 text-muted-foreground transition-transform', !isExpanded && '-rotate-90')} />
              </button>
              {isExpanded && <div className="space-y-3 px-2 pb-3 pt-1">
                {sections.map((section) => {
                  const sectionItems = groupItems.filter((item) => (item.section || '') === section);
                  return <div key={section || group}>
                    {section && sections.length > 1 && <p className="mb-1 px-2 text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground/80">{section}</p>}
                    <div className="space-y-0.5">{sectionItems.map((item) => {
                      const Icon = item.icon; const active = isItemActive(pathname, item.href);
                      return <button type="button" key={item.href} onClick={() => handleNavigation(item.href)} aria-current={active ? 'page' : undefined} className={cn('flex min-h-10 w-full items-center gap-3 rounded-lg px-2.5 py-2 text-left text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring', active ? 'bg-primary text-primary-foreground shadow-sm' : 'text-sidebar-foreground/85 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground')}>
                        <Icon className="h-4 w-4 shrink-0" /><span className="min-w-0 flex-1 truncate">{item.label}</span>
                      </button>;
                    })}</div>
                  </div>;
                })}
      ...
      <div className="border-t border-sidebar-border p-3">
        <div className="rounded-xl bg-muted/35 p-2"><ThemeToggle /><Button variant="ghost" className="mt-1 w-full justify-start gap-3 text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={logout}><LogOut className="h-4 w-4" />Cerrar sesión</Button></div>
        <p className="mt-3 text-center text-[10px] text-muted-foreground">Tecnología operacional por <a href="https://n3uralia.com" target="_blank" rel="noopener noreferrer" className="font-semibold hover:text-foreground">N3uralia</a></p>
      </div>
    </aside>
    {isOpen && <button type="button" aria-label="Cerrar navegación" className="fixed inset-0 z-30 bg-black/50 lg:hidden" onClick={() => setIsOpen(false)} />}
  </>;
}
