...
export function Sidebar() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return menuItems.filter((item) => {
      const roleAllowed = role === 'superadmin' || role === 'admin' || !item.roles || item.roles.includes(role);
      if (!roleAllowed) return false;
      if (!enforced || !item.moduleKey) return true;
      return canView(item.moduleKey);
    });
  }, [role, enforced, canView]);

  const activeGroup = useMemo(
    () => filteredItems.find((item) => isItemActive(pathname, item.href))?.group,
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-40 flex h-screen w-[252px] flex-col border-r border-sidebar-border bg-sidebar transition-transform duration-200 lg:static lg:translate-x-0',
          isOpen ? 'translate-x-0' : '-translate-x-full',
        )}
      >
        <div className="border-b border-sidebar-border px-4 py-4">
          <Link href="/dashboard" className="flex items-center gap-3 rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
        ...
        <nav className="flex-1 overflow-y-auto px-2.5 py-3" aria-label="Navegación principal">
              ...
              return (
                <section key={group}>
                  <button
                    ...
                    className="flex w-full items-center justify-between rounded-md px-2 py-2 text-left text-xs font-semibold text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground"
                  >
                    <span>{group}</span>
                      ...
                      {items.map((item) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              'flex min-h-9 w-full items-center gap-2.5 rounded-md px-2.5 py-2 text-left text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring',
                              active
        ...
        <div className="border-t border-sidebar-border p-2.5">
          ...
          <Button
            variant="ghost"
            className="mt-1 w-full justify-start gap-2.5 text-destructive hover:bg-destructive/10 hover:text-destructive"
            onClick={logout}
          >
