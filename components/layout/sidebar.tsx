...
type MenuItem = { label: string; href: string; icon: LucideIcon; group: string; moduleKey?: string; roles?: string[]; pedroOnly?: boolean };
...
export function Sidebar(){
  ...
  const filteredItems=useMemo(()=>{if(!role)return[];return menuItems.filter((item)=>{if(item.pedroOnly&&!isPedro)return false;const roleAllowed=role==='superadmin'||role==='admin'||(role==='gerente_operaciones'&&item.group!=='Administración')||!item.roles||item.roles.includes(role);if(!roleAllowed)return false;if(!enforced||!item.moduleKey)return true;return canView(item.moduleKey)})},[role,isPedro,enforced,canView]);
  ...
  return <><Button variant="outline" size="icon" aria-label={isOpen?'Cerrar navegación':'Abrir navegación'} className="fixed left-4 top-3 z-50 bg-background lg:hidden" onClick={()=>setIsOpen((current)=>!current)}>{isOpen?<X className="h-5 w-5"/>:<Menu className="h-5 w-5"/>}</Button><aside className={cn('fixed inset-y-0 left-0 z-40 flex h-screen w-[248px] flex-col border-r border-sidebar-border/80 bg-sidebar transition-transform duration-200 lg:static lg:translate-x-0',isOpen?'translate-x-0':'-translate-x-full')}><div className="border-b border-sidebar-border/80 px-4 py-4"><Link href="/dashboard" className="flex items-center gap-3 rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><div className="flex h-9 w-9 items-center justify-center rounded-md border border-primary/25 bg-primary/10 text-primary"><Activity className="h-4 w-4"/></div><div className="min-w-0"><p className="font-heading text-sm font-semibold tracking-tight">MOTIL</p><p className="truncate text-[10px] uppercase tracking-[0.09em] text-muted-foreground">Mining Operating System</p></div></Link></div><nav className="flex-1 overflow-y-auto px-3 py-3" aria-label="Navegación principal"><div className="space-y-2">{groupOrder.map((group)=>{const items=filteredItems.filter((item)=>item.group===group);if(!items.length)return null;const expanded=expandedGroups[group]??false;return <section key={group}><button type="button" onClick={()=>setExpandedGroups((current)=>({...current,[group]:!expanded}))} aria-expanded={expanded} className="flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left text-[10px] font-semibold uppercase tracking-[0.1em] text-muted-foreground transition-colors hover:text-foreground"><span>{group}</span><ChevronDown className={cn('h-3.5 w-3.5 transition-transform',!expanded&&'-rotate-90')}/></button>{expanded?<div className="mt-0.5 space-y-0.5">{items.map((item)=>{const Icon=item.icon;const active=isItemActive(pathname,item.href);return <button type="button" key={item.href} onClick={()=>navigate(item.href)} aria-current={active?'page':undefined} className={cn('relative flex min-h-9 w-full items-center gap-2.5 rounded-md px-2.5 py-2 text-left text-[13px] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring',active?'bg-primary/10 font-medium text-foreground before:absolute before:inset-y-2 before:left-0 before:w-0.5 before:rounded-full before:bg-primary':'text-sidebar-foreground/72 hover:bg-sidebar-accent/70 hover:text-sidebar-accent-foreground')}><Icon className={cn('h-4 w-4 shrink-0',active?'text-primary':'text-muted-foreground')}/><span className="truncate">{item.label}</span></button>})}</div>:null}</section>})}</div></nav><div className="border-t border-sidebar-border/80 p-3"><ThemeToggle/><Button variant="ghost" className="mt-1 w-full justify-start gap-2.5 text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={logout}><LogOut className="h-4 w-4"/>Cerrar sesión</Button></div></aside>{isOpen?<button type="button" aria-label="Cerrar navegación" className="fixed inset-0 z-30 bg-black/45 lg:hidden" onClick={()=>setIsOpen(false)}/>:null}</>;
}
