...
const number = (value: unknown, digits = 0) => Number(value).toLocaleString('es-CL', { maximumFractionDigits: digits });
const money = (value: unknown) => value == null ? 'Sin base' : `$${Number(value).toLocaleString('es-CL', { maximumFractionDigits: 0 })}`;
...
export function Asset360Overview({ assetId, scope = 'equipos' }: { assetId: string; scope?: 'equipos' | 'vehiculos' }) {
  ...
  const actionableWorkOrder = data.closeReadiness?.find((row) => row.work_order_id && !row.ready_to_close) || data.closeReadiness?.find((row) => row.work_order_id) || null;
  const mtbf = Number(rr?.valid_mtbf_intervals || 0) > 0 && rr?.mtbf_operating_hours != null ? `${number(rr.mtbf_operating_hours, 1)} h` : 'Sin base';
  const mttr = Number(rr?.audited_corrective_events || 0) > 0 && rr?.mttr_hours != null ? `${number(rr.mttr_hours, 1)} h` : 'Sin base';
    ...
    <Card className="shadow-none">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          ...
          <div className="rounded-lg border p-4"><p className="text-xs font-medium text-muted-foreground">Confiabilidad auditada</p><div className="mt-2 grid grid-cols-2 gap-3"><div><p className="text-xs text-muted-foreground">MTTR</p><p className="font-medium">{mttr}</p></div><div><p className="text-xs text-muted-foreground">Cobertura horómetro</p><p className="font-medium">{Number(rr?.audited_corrective_events || 0) > 0 && rr?.meter_event_coverage_percent != null ? `${number(rr.meter_event_coverage_percent, 0)}%` : 'Sin base'}</p></div><div><p className="text-xs text-muted-foreground">Cierres auditados</p><p className="font-medium">{Number(reliability?.audited_closures || 0)}</p></div><div><p className="text-xs text-muted-foreground">Causas recurrentes</p><p className="font-medium">{Number(reliability?.recurring_cause_count || 0)}</p></div></div><Button asChild variant="ghost" size="sm" className="mt-3 px-0"><Link href="/dashboard/mantenimiento/confiabilidad">Ver confiabilidad<ArrowRight className="ml-1 h-4 w-4" /></Link></Button></div>

          <div className="rounded-lg border p-4"><p className="text-xs font-medium text-muted-foreground">Cierre y ejecución</p><div className="mt-2 grid grid-cols-2 gap-3"><div><p className="text-xs text-muted-foreground">Pasos pendientes</p><p className="font-medium">{summary.pendingPlanSteps}</p></div><div><p className="text-xs text-muted-foreground">Listas para cerrar</p><p className="font-medium">{summary.readyToClose}</p></div><div><p className="text-xs text-muted-foreground">Críticas abiertas</p><p className="font-medium">{summary.criticalOpen}</p></div><div><p className="text-xs text-muted-foreground">Reinicios horómetro</p><p className="font-medium">{runtime ? Number(runtime.reset_count || 0) : 'Sin lectura'}</p></div></div>{actionableWorkOrder ? <Button asChild variant="ghost" size="sm" className="mt-3 px-0"><Link href={`/dashboard/mantenimiento/ordenes-trabajo/cierre?workOrderId=${encodeURIComponent(actionableWorkOrder.work_order_id)}`}>Continuar trabajo<ArrowRight className="ml-1 h-4 w-4" /></Link></Button> : <p className="mt-3 text-xs text-muted-foreground">No hay una OT activa con cierre pendiente para continuar.</p>}</div>
        </div>

        <div className="rounded-lg border bg-muted/20 p-3 text-xs text-muted-foreground">El horómetro proviene de lecturas runtime del activo. MTBF y MTTR sólo aparecen cuando existe evidencia correctiva auditada suficiente. El costo corresponde a snapshots de cierre auditado; no se mezcla con el ledger histórico importado.</div>
