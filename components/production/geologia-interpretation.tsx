...
export function GeologiaInterpretation(){
  ...
  if(isLoading||!data) return <StatePanel title="Cargando interpretación" description="Consolidando intervalos fuente, puntos, transiciones y evidencia operacional." className="min-h-0 py-5"/>;
  ...
  return <div className="space-y-5">
    <section className="border-b pb-5">
      ...
      <p className="mt-2 max-w-3xl text-sm text-muted-foreground">Lectura técnica sobre evidencia canónica y operacional. Los intervalos estructurados actuales provienen de reportes de perforación y no reemplazan logging geológico formal, survey, ensayes ni validación del geólogo.</p>
      <div className="mt-5 flex flex-wrap gap-x-6 gap-y-2 text-sm"><span><strong>{s.structured_evidence||0}</strong> con evidencia estructurada</span><span><strong>{s.operational_evidence||0}</strong> operacionales</span><span><strong>{s.partial_evidence||0}</strong> parciales</span><span><strong>{s.blocked||0}</strong> bloqueados</span><span><strong>{s.insufficient_evidence||0}</strong> insuficientes</span>{data.patternCount?<span><strong>{data.patternCount}</strong> patrones observados</span>:null}{data.hypothesisCount?<span><strong>{data.hypothesisCount}</strong> hipótesis · {hs.in_review||0} en revisión</span>:null}</div>
    </section>
...
function InterpretationDetail({data}:{data:DetailResponse}){
  ...
  if(r.structured_intervals>0) strengths.push(`${r.structured_intervals} intervalos operacionales estructurados`);
  ...
  if(r.structured_intervals===0) limitations.push('No existe evidencia intervalar estructurada suficiente para revisar relaciones por profundidad.');
  if(r.structured_intervals>0) limitations.push('Los intervalos estructurados actuales son evidencia operacional de reportes de perforación; no equivalen a logging geológico formal, RQD, recuperación, alteración validada ni contactos geológicos cerrados.');
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-lg border bg-card p-5"><p className="font-medium">Señales principales</p><div className="mt-3"><SignalLine label="Mineralización visual" value={`${n(r.visual_mineral_m)} m`} detail={`${r.structured_mineral_intervals} intervalos operacionales · ${r.mineral_points} puntos`}/><SignalLine label="Ausencia explícita" value={`${n(r.explicit_no_mineral_m)} m`} detail="Observación operacional negativa; no prueba esterilidad fuera del tramo observado."/><SignalLine label="Estructuras" value={`${n(r.structure_m)} m`} detail={`${r.structured_structure_intervals} intervalos operacionales · ${r.structure_points} puntos estructurales`}/><SignalLine label="Litología reportada" value={`${n(r.lithology_m)} m`} detail={`${r.structured_lithology_intervals} intervalos operacionales · ${r.transition_points} transiciones`}/><SignalLine label="Condición de roca" value={`${n(r.rock_condition_m)} m`} detail="Evidencia operacional de dureza, fracturamiento u otras condiciones descritas por perforación."/></div></section>
