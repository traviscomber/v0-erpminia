...
const supplementalFetcher=async(url:string):Promise<SupplementalData>=>{
  ...
  if(!response.ok)throw new Error(data.error||'No fue posible cargar resultados químicos canónicos');
  return data;
};
...
export function GeologiaResultsDecisionBoard({samples,holes,samplesValidated,samplesReview,assaysCanonical}:Props){
  ...
  const historicalPunteoOrProcess=samples.filter((sample)=>['special_punteo','process_special'].includes(String(sample.sample_type||'').toLowerCase())).length;
  ...
  return <div className="space-y-5">
    <section className="rounded-lg border bg-card p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm text-muted-foreground">Los registros químicos actuales son evidencia histórica de muestra/mina. La validación del registro no los convierte en muestras de sondaje: sólo existe atribución por pozo o intervalo cuando `drill_hole_id` y profundidad están explícitamente respaldados.</p>
        </div>
        <ShieldCheck className="h-5 w-5 text-muted-foreground"/>
      </div>

      <div className="mt-5 grid gap-px overflow-hidden rounded-lg border bg-border sm:grid-cols-2 xl:grid-cols-4">
        <Metric label="Registros químicos" value={samples.length} detail={`${historicalPunteoOrProcess} punteo/proceso histórico`}/>
        <Metric label="Validados" value={`${validationPct}%`} detail={`${samplesValidated}/${samples.length} con validación de registro`}/>
        <Metric label="Ligados a sondaje" value={`${holeLinked}/${samples.length}`} detail="Vínculo explícito hacia pozo"/>
        <Metric label="Con intervalo" value={`${intervalLinked}/${samples.length}`} detail="Desde / hasta explícitos"/>
      </div>
    </section>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,.85fr)]">
      <section className="rounded-lg border bg-card p-5">
        ...
        <div className="mt-4 space-y-4 text-sm">
          <div className="flex gap-3"><Beaker className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground"/><div><p className="font-medium">Ley histórica</p><p className="mt-1 text-muted-foreground">{assaysCanonical?`Existen resultados químicos canónicos históricos. Se pueden leer por muestra/mina con valor, unidad, fecha y fuente; ${assaysLinkedToHole}/${assays.length} están ligados a sondaje. No usar el resto como ley por pozo, espesor o continuidad.`:'La vista actual no dispone de resultados químicos canónicos utilizables para interpretar ley. Motil debe limitarse a trazabilidad y contexto; no estimar ni inferir Cu, espesor mineralizado o continuidad.'}</p></div></div>
          <div className="flex gap-3"><Link2 className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground"/><div><p className="font-medium">Trazabilidad</p><p className="mt-1 text-muted-foreground">{holeLinked===samples.length&&samples.length>0?'Todos los registros químicos visibles están vinculados explícitamente a sondaje.':`${samples.length-holeLinked} registros químicos no están vinculados a un sondaje canónico; no deben atribuirse a un pozo por fecha, mina, sector, nombre o proximidad.`}</p></div></div>
          <div className="flex gap-3"><Ruler className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground"/><div><p className="font-medium">Intervalos</p><p className="mt-1 text-muted-foreground">{intervalLinked===samples.length&&samples.length>0?'Todos los registros tienen profundidad desde/hasta explícita.':`${samples.length-intervalLinked} registros químicos no tienen intervalo completo. Sin profundidad no corresponde atribuirlos a un tramo del sondaje.`}</p></div></div>
          <div className="flex gap-3"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground"/><div><p className="font-medium">QA / validación</p><p className="mt-1 text-muted-foreground">{samplesReview>0?`${samplesReview} registros químicos requieren revisión. Deben quedar fuera de conclusiones cerradas hasta resolver su estado.`:'No hay registros químicos marcados para revisión. Ese estado valida la fila importada, no su pertenencia a un sondaje.'}</p></div></div>
        </div>
      </section>

      <section className="rounded-lg border bg-card p-5">
        <div className="flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-muted-foreground"/><p className="font-medium">Qué falta para una lectura geológica por sondaje</p></div>
        <p className="mt-3 text-sm text-muted-foreground">Los resultados históricos de Cu existen, pero hoy no están vinculados canónicamente a sondajes ni intervalos. Son evidencia histórica de muestra/mina —principalmente punteo/proceso—, no continuidad mineralizada por pozo.</p>
        <div className="mt-4 rounded-lg border bg-muted/10 p-4 text-sm"><p className="font-medium">Regla de producto</p><p className="mt-1 text-muted-foreground">No mezclar ley de cabeza, punteo, muestra de proceso, ensayo de sondaje, ley planificada u otras medidas sin un vínculo y una regla operacional explícitos.</p></div>
        <p className="mt-4 text-xs text-muted-foreground">{sourceLinked}/{samples.length} registros conservan referencia de archivo o planilla fuente.</p>
      </section>
    </div>

    {sorted.length?<section className="overflow-hidden rounded-lg border bg-card">
      <div className="border-b px-4 py-3"><p className="font-medium">Registros químicos históricos · más recientes primero</p><p className="mt-1 text-sm text-muted-foreground">Trazabilidad de muestra/mina. La fecha más nueva siempre aparece arriba cuando está disponible.</p></div>
    ...
    </section>:<div className="rounded-lg border border-dashed bg-muted/10 px-5 py-8 text-center"><p className="font-medium">Sin registros químicos cargados</p><p className="mx-auto mt-2 max-w-xl text-sm text-muted-foreground">Los resultados aparecerán cuando exista evidencia química canónica vinculada a la operación.</p></div>}

    {assays.length?<section className="overflow-hidden rounded-lg border bg-card">
      <div className="border-b px-4 py-3"><div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">Resultados químicos canónicos históricos · más recientes primero</p><p className="mt-1 text-sm text-muted-foreground">Resultados reales de La Patagua. Son evidencia histórica de muestra/mina y no representan por sí solos la condición geológica vigente de 2026 ni una ley por sondaje.</p></div><p className="text-xs text-muted-foreground">{assaysValidated}/{assays.length} validados · {assaysLinkedToHole} ligados a sondaje</p></div></div>
    ...
    </section>:assaysCanonical&&!assayError?<div className="rounded-lg border border-dashed bg-muted/10 px-5 py-6 text-sm text-muted-foreground">Cargando resultados químicos canónicos…</div>:null}
  </div>;
}
