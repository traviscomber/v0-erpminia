  ...
  return <section className="space-y-6 rounded-lg border bg-card p-5" aria-label="Histórico canónico de Geología">
    <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
      <div>
        ...
        <p className="mt-1 max-w-3xl text-sm text-muted-foreground">Ordenado desde el dato más vigente hacia atrás. Plan, ley de planta, sondajes y química histórica conservan su fuente y granularidad; Motil sólo relaciona registros cuando existe vínculo canónico explícito.</p>
      </div>
    ...
    <div>
      <div className="mb-3"><p className="font-medium">4. Química histórica · 2016–2017</p><p className="mt-1 text-xs text-muted-foreground">Se muestra después de la operación vigente porque corresponde a evidencia histórica más antigua de muestra/mina. No representa por sí sola una ley de sondaje.</p></div>
      ...
      {results.length?<div className="mt-3 overflow-hidden rounded-md border"><div className="border-b px-4 py-3"><div className="flex items-center gap-2"><Beaker className="h-4 w-4 text-muted-foreground"/><p className="font-medium">Resultados químicos · más reciente primero</p></div><p className="mt-1 text-xs text-muted-foreground">Último {formatDate(lastAssay)} · desde {formatDate(firstAssay)}. Un pozo aparece sólo cuando el dato histórico lo identifica explícitamente.</p></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-muted/30 text-left text-xs text-muted-foreground"><tr><th className="px-4 py-3">Fecha</th><th className="px-4 py-3">Muestra</th><th className="px-4 py-3">Mina</th><th className="px-4 py-3">Analito</th><th className="px-4 py-3 text-right">Resultado</th><th className="px-4 py-3">Método</th><th className="px-4 py-3">Fuente</th><th className="px-4 py-3">Vínculo pozo</th></tr></thead><tbody className="divide-y">{results.map((row)=><tr key={row.id}><td className="whitespace-nowrap px-4 py-3">{formatDate(row.sample_date||row.result_date)}</td><td className="px-4 py-3 font-medium">{row.sample_code||'—'}</td><td className="px-4 py-3">{row.mine_name||'Sin mina canónica'}</td><td className="px-4 py-3">{row.analyte_name||row.analyte_code||'—'}</td><td className="px-4 py-3 text-right tabular-nums">{cu(row.result_value,row.result_unit)}</td><td className="px-4 py-3">{row.method_code||'—'}</td><td className="px-4 py-3 text-xs text-muted-foreground">{row.source_file||'—'}{row.source_sheet?` · ${row.source_sheet}`:''}</td><td className="px-4 py-3 text-xs text-muted-foreground">{row.drill_hole_id?'Vínculo explícito':'No asignado'}</td></tr>)}</tbody></table></div></div>:null}
    </div>

    <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-center">
      <div className="rounded-md bg-muted/30 px-4 py-3 text-sm"><p className="font-medium">Regla temporal y de integración</p><p className="mt-1 text-muted-foreground">La lectura siempre prioriza el dato más reciente. La ley cabeza corresponde a operación de planta; los resultados químicos históricos conservan muestra/mina real; el plan conserva mina/sector/actividad. {linkedToHole===0?'Los resultados químicos sin sondaje explícito no se adhieren a pozos por similitud o fecha.':`${linkedToHole} resultados químicos tienen vínculo explícito a sondaje.`}</p></div>
