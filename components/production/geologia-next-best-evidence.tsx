...
export function GeologiaNextBestEvidence() {
  ...
  if (error) return <StatePanel tone="error" title="No fue posible leer prioridades" description="La evidencia canónica y la matriz permanecen disponibles; falló sólo esta capa derivada." className="min-h-0 py-5" />;
  if (isLoading || !data) return <StatePanel title="Leyendo evidencia canónica" description="Separando excepciones accionables de límites estructurales de la fuente." className="min-h-0 py-5" />;

  return <div className="space-y-5">
    <section className="border-b pb-5">
      <p className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">Prioridad canónica</p>
      <h2 className="mt-2 text-2xl font-semibold tracking-tight">Trabajar con lo que sí tenemos</h2>
      <p className="mt-2 max-w-4xl text-sm text-muted-foreground">MOTIL no transforma ausencia masiva de datos en una lista de tareas. Si una dimensión aparece en menos de {data.source_boundary_threshold_pct}% de los {data.total_holes} sondajes, se considera un límite conocido de la fuente canónica actual. Sólo se priorizan excepciones cuando existe cobertura suficiente para que la acción tenga sentido.</p>
      <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-sm">
        <span><strong>{data.source_pattern_count}</strong> patrones canónicos considerados</span>
        <span><strong>{data.rows.length}</strong> faltantes realmente accionables</span>
    ...
    {data.rows.length ? <>
      <section className="grid gap-4 lg:grid-cols-3">
        {data.top.slice(0, 3).map((row, index) => <article key={row.evidence} className="rounded-lg border bg-card p-5">
          <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Prioridad {index + 1} · {priorityLabel[row.recovery_priority]}</p>
      ...
      <section className="overflow-hidden rounded-lg border bg-card">
        <div className="border-b px-5 py-4"><div className="flex items-center gap-2"><CircleDot className="h-4 w-4" /><p className="font-medium">Excepciones accionables</p></div></div>
    ...
    </> : <StatePanel title="Sin deuda de datos accionable" description="Con la cobertura canónica actual no corresponde pedir datos masivamente. Geología debe operar con la evidencia disponible y mantener sus límites explícitos." className="min-h-0 py-5" />}
