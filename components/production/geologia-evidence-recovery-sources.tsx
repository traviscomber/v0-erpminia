...
export function GeologiaEvidenceRecoverySources() {
  ...
  return <div className="space-y-5 border-t pt-6">
    ...
    <section className="grid gap-4 md:grid-cols-3">
      ...
      <article className="rounded-lg border bg-card p-5">
        <div className="flex items-center gap-2 text-sm font-medium"><FileSearch className="h-4 w-4" />Logging por recuperar</div>
        <p className="mt-3 text-2xl font-semibold tabular-nums">{data.summary.logging_clue_holes}</p>
        <p className="mt-1 text-xs text-muted-foreground">sondajes sin intervalos canónicos pero con pistas litológicas históricas</p>
        <p className="mt-3 text-sm"><strong>{data.summary.structural_clue_holes}</strong> sondajes con menciones estructurales fuente.</p>
      </article>
      <article className="rounded-lg border bg-card p-5">
