...
import { AlertTriangle, Beaker, CheckCircle2, Link2, Microscope } from 'lucide-react';
...
type ViewKey='process'|'historical';
...
export function ChemistryDashboard(){
  ...
  const [view,setView]=useState<ViewKey>('process');
  const p=data?.period;
  ...
  const cards=[
    ...
    {label:'Turnos ensayados',value:p?n(p.assayed):'—',detail:p?`${n(p.totalShifts)} turnos en el período`:'—',icon:Beaker},
    {label:'Sin ensayo',value:p?n(p.pending):'—',detail:'Evidencia analítica pendiente; no se imputa',icon:AlertTriangle},
    {label:'Último dato',value:p?date(p.dataThrough):'—',detail:'Fecha máxima de evidencia de proceso',icon:Microscope},
  ];

  return <div className="space-y-6">
    <PageHeader><PageHeaderContent><PageHeaderEyebrow>Producción · Química</PageHeaderEyebrow><PageHeaderTitle>Química</PageHeaderTitle><PageHeaderDescription>Dos contextos analíticos separados: ensayos operacionales de Planta/Metalurgia y muestras especiales históricas de mina. No se mezclan para interpretar ley.</PageHeaderDescription></PageHeaderContent></PageHeader>
    ...
    <section className="border-b pb-3" aria-label="Contexto de Química">
      <div className="flex flex-wrap items-center gap-1" role="tablist" aria-label="Vista de Química">
        <Button size="sm" variant="ghost" role="tab" aria-selected={view==='process'} onClick={()=>setView('process')} className={`h-8 px-2.5 text-xs ${view==='process'?'bg-muted text-foreground':'text-muted-foreground'}`}>Proceso</Button>
        <Button size="sm" variant="ghost" role="tab" aria-selected={view==='historical'} onClick={()=>setView('historical')} className={`h-8 px-2.5 text-xs ${view==='historical'?'bg-muted text-foreground':'text-muted-foreground'}`}>Mina histórica</Button>
      </div>
      <p className="mt-2 text-xs text-muted-foreground">{view==='process'?(p?`Evidencia analítica de Planta/Metalurgia disponible hasta ${date(p.dataThrough)}.`:'Ensayos de proceso de Planta/Metalurgia.'):'Muestras especiales 2016–2017 · contexto histórico; no son ensayes de sondaje ni ley representativa del plan 2026.'}</p>
    </section>

    {view==='process'?<>
      <section className="rounded-lg border bg-muted/20 px-4 py-3 text-sm"><p className="font-medium">Química controla la evidencia analítica</p><p className="mt-1 text-muted-foreground">Ley cabeza, recuperación, fino Cu y plan vs ejecución se consolidan en Planta / Metalurgia. Aquí se controla cobertura de ensayos, faltantes, validación y procedencia de resultados.</p></section>

      <section className="grid gap-px overflow-hidden rounded-lg border bg-border sm:grid-cols-2 xl:grid-cols-4" aria-label="Control analítico de proceso">{cards.map(c=>{const Icon=c.icon;return <div key={c.label} className="bg-card px-5 py-4"><div className="flex items-center justify-between gap-3"><p className="text-xs text-muted-foreground">{c.label}</p><Icon className="h-4 w-4 text-muted-foreground"/></div><p className="mt-2 text-2xl font-semibold tracking-tight">{isLoading?'—':c.value}</p><p className="mt-1 text-xs text-muted-foreground">{c.detail}</p></div>})}</section>

      {data?.pending?.length?<section className="rounded-lg border bg-card"><div className="border-b px-4 py-3"><h2 className="font-medium">Ensayos de proceso pendientes</h2><p className="mt-1 text-xs text-muted-foreground">Turnos con evidencia incompleta; no se imputan valores.</p></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left text-xs text-muted-foreground"><th className="px-4 py-3 font-medium">Fecha</th><th className="px-4 py-3 font-medium">Turno</th><th className="px-4 py-3 font-medium">Estado</th><th className="px-4 py-3 font-medium">Fuente</th></tr></thead><tbody>{data.pending.map(r=><tr key={r.plantShiftId} className="border-b last:border-0"><td className="px-4 py-3">{date(r.operationDate)}</td><td className="px-4 py-3">{r.shiftCode||'—'}</td><td className="px-4 py-3">{r.state}</td><td className="px-4 py-3 text-muted-foreground">{r.sourceFile||'—'}{r.sourceRow?` · fila ${r.sourceRow}`:''}</td></tr>)}</tbody></table></div></section>:data&&!isLoading?<StatePanel title="Sin ensayos de proceso pendientes" description="No hay turnos abiertos por falta de evidencia analítica en el período disponible." className="min-h-0 py-5"/>:null}

      {data?.recent?.length?<section className="rounded-lg border bg-card"><div className="border-b px-4 py-3"><h2 className="font-medium">Resultados analíticos recientes</h2><p className="mt-1 text-xs text-muted-foreground">Resultados fuente de Planta/Metalurgia. Su lectura operacional consolidada vive en Planta / Metalurgia; aquí se conserva la evidencia y validación.</p></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left text-xs text-muted-foreground"><th className="px-4 py-3 font-medium">Cabeza</th><th className="px-4 py-3 font-medium">Concentrado</th><th className="px-4 py-3 font-medium">Relave</th><th className="px-4 py-3 font-medium">Recuperación reportada</th><th className="px-4 py-3 font-medium">Validación</th><th className="px-4 py-3 font-medium">Fuente</th></tr></thead><tbody>{data.recent.map(r=><tr key={r.id} className="border-b last:border-0"><td className="px-4 py-3">{pct(r.head_grade,3)}</td><td className="px-4 py-3">{pct(r.concentrate_grade,2)}</td><td className="px-4 py-3">{pct(r.tailings_grade,3)}</td><td className="px-4 py-3">{pct(r.recovery_reported,2)}</td><td className="px-4 py-3">{r.validation_status||r.analysis_status||'—'}</td><td className="px-4 py-3 text-muted-foreground">{r.source_file||'—'}{r.source_row?` · fila ${r.source_row}`:''}</td></tr>)}</tbody></table></div></section>:data&&!isLoading?<StatePanel title="Sin resultados de proceso en el período" description="No se muestran valores estimados ni se sustituyen ensayos faltantes con química histórica." className="min-h-0 py-5"/>:null}

      {data?.lineage?<div className="flex items-start gap-3 rounded-lg border bg-card px-4 py-3 text-xs text-muted-foreground"><Link2 className="mt-0.5 h-4 w-4 shrink-0"/><div><strong className="font-medium text-foreground">Linaje de proceso:</strong> {data.lineage.processAssays}</div></div>:null}
    </>:null}

    {view==='historical'?<>
      <section className="rounded-lg border bg-muted/20 px-4 py-3 text-sm"><p className="font-medium">Contexto histórico, no ley operacional actual</p><p className="mt-1 text-muted-foreground">Muestras especiales 2016–2017 provenientes de `LEY (1).xlsx`. No son ensayes de sondaje, no representan la ley del plan 2026 y no completan automáticamente relaciones muestra → pozo → intervalo.</p></section>

      <section className="grid gap-px overflow-hidden rounded-lg border bg-border sm:grid-cols-2 xl:grid-cols-5" aria-label="Química histórica de mina">
        ...
        <Metric label="Pozos vinculados" value={canonical?n(canonical.holes_with_samples):'—'} detail="Sólo vínculo demostrable"/>
      ...
      </section>:data&&!isLoading?<StatePanel title="Sin química histórica sectorizada" description="No se inventan sectores, pozos ni valores para completar cobertura." className="min-h-0 py-5"/>:null}
      ...
      {data?.lineage?<div className="flex items-start gap-3 rounded-lg border bg-card px-4 py-3 text-xs text-muted-foreground"><Link2 className="mt-0.5 h-4 w-4 shrink-0"/><div><strong className="font-medium text-foreground">Linaje histórico:</strong> {data.lineage.targetLineage}. <span>{data.lineage.note}</span></div></div>:null}
    </>:null}
  </div>;
}
