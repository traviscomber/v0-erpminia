...
export function ProductionFineCloseoutGate(){
  ...
  return <section className="rounded-lg border bg-card">
    <div className="flex items-center justify-between gap-3 border-b px-4 py-3"><div className="flex items-center gap-2"><PackageCheck className="h-4 w-4 text-muted-foreground"/><h2 className="font-medium">Cierre de Producción fino</h2></div><span className="text-xs text-muted-foreground">{ready?'Reconciliable':'Pendiente de fuente'}</span></div>
    <div className="grid gap-px bg-border md:grid-cols-3">
      ...
      <div className="bg-card px-4 py-4"><p className="text-xs text-muted-foreground">Estado de cierre</p><div className="mt-2 flex items-center gap-2">{ready?<CheckCircle2 className="h-5 w-5"/>:<AlertTriangle className="h-5 w-5"/>}<p className="text-xl font-semibold">{ready?'Listo':'Bloqueado'}</p></div><p className="mt-1 text-xs text-muted-foreground">{ready?'Existe evidencia para reconciliar':'Falta evidencia de concentrado/despacho'}</p></div>
    </div>
