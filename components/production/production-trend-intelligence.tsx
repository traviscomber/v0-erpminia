...
import { AlertTriangle, ArrowDownRight, ArrowRight, ArrowUpRight, Gauge } from 'lucide-react';
...
type Payload={method:{window:string;policy:string};metrics:{treatedTons:TrendMetric;headGrade:TrendMetric;recovery:TrendMetric;drillingMeters:TrendMetric;drillingOperationalRate:TrendMetric;availability:TrendMetric};equipmentStateSeries:Array<{date:string;reportedAssets:number;operationalAssets:number;outOfServiceAssets:number;operationalRatePct:number}>;semantics:{drillingOperationalRate:string};alerts:Array<{key:string;severity:'warning'|'info';title:string;evidence:string;action:string}>};
...
export function ProductionTrendIntelligence(){
  ...
  const cards=[
    ...
    ['Equipos operativos reportados',data.metrics.drillingOperationalRate,'%',1],
    ['Disponibilidad mecánica',data.metrics.availability,'%',1],
  ] as const;
  const latestEquipment=data.equipmentStateSeries[data.equipmentStateSeries.length-1]||null;
  return <section className="space-y-3">
    ...
    <div className="grid gap-px overflow-hidden rounded-lg border bg-border sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">{cards.map(([label,metric,unit,digits])=><TrendCard key={label} label={label} metric={metric} unit={unit} digits={digits}/>)}</div>
    {latestEquipment?<div className="rounded-lg border bg-card px-4 py-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-medium">Estado diario de equipos de Sondaje</p><p className="mt-1 text-xs text-muted-foreground">{data.semantics.drillingOperationalRate}</p></div><p className="text-sm"><strong>{latestEquipment.operationalAssets}</strong> operativos · <strong>{latestEquipment.outOfServiceAssets}</strong> fuera de servicio · {date(latestEquipment.date)}</p></div></div>:null}
...
function TrendCard({label,metric,unit,digits}:{label:string;metric:TrendMetric;unit:string;digits:number}){
  ...
  return <div className="bg-card p-4"><div className="flex items-center justify-between gap-2"><p className="text-xs text-muted-foreground">{label}</p><Icon className="h-4 w-4 shrink-0 text-muted-foreground"/></div><p className="mt-2 text-xl font-semibold">{metric.recent===null?'—':`${n(metric.recent,digits)} ${unit}`}</p><p className="mt-1 text-xs font-medium">{state}</p>{metric.prior!==null?<p className="mt-1 text-xs text-muted-foreground">Previo {n(metric.prior,digits)} {unit}</p>:null}</div>;
}
