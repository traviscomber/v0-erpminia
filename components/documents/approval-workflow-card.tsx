...
export function ApprovalWorkflowCard({
  ...
  const getStepIcon = (status: string) => {
    switch (status) {
      case 'approved':
        return <CheckCircle className="h-6 w-6 text-[var(--brand-verde)]" />;
      case 'rejected':
  ...
  const handleApprove = async (step: ApprovalStep) => {
    ...
    try {
      ...
      toast.success(`${step.levelName} aprobado correctamente`);
      setComments((prev) => ({ ...prev, [step.id]: '' }));
    } catch (error) {
      console.error('[ApprovalWorkflowCard] Error al aprobar:', error);
            ...
            <div key={step.id} className="space-y-3">
              <div className="flex items-start gap-4">
                <div className="flex flex-col items-center">
                  ...
                  {index < steps.length - 1 && <div className="mt-2 h-12 w-0.5 bg-muted" />}
                </div>
