...
export function EeccImportXls() {
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch('/api/eecc/import', {
        method: 'POST',
        credentials: 'include',
        body: formData,
      });
      ...
      if (!response.ok) {
        setResult({
          success: false,
          message: 'No se pudieron importar los EECC',
      ...
      setResult({
        success: true,
        message: payload.message || 'Empresas importadas correctamente',
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          ...
          Importar Empresas Contratistas (EECC)
        </CardTitle>
      </CardHeader>
        ...
        <Alert>
          ...
          <AlertDescription>
            ...
            <div className="rounded bg-muted p-2 font-mono text-sm">
              EMPRESA | RUT | REPRESENTANTE | CORREO | TELÉFONO (opcional)
            </div>
          </AlertDescription>
        </Alert>
