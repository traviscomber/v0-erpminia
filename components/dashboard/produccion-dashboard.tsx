  ...
  return (
    <div className="space-y-6">
      <PageHeader>
        <PageHeaderContent>
          ...
          <PageHeaderDescription>
            Producción integra Transporte de Mineral, Planta/Metalurgia, Geología, Topografía, Química y Sondaje. La sensorización pertenece exclusivamente a Maestranza.
          </PageHeaderDescription>
        </PageHeaderContent>
        <PageHeaderActions>
        ...
        {domains.map((domain) => (
          <Link key={domain.href} href={domain.href} className="block">
            <Card className="h-full transition-colors hover:border-primary/50">
              <CardHeader className="pb-3"><CardTitle className="flex items-center justify-between text-base">{domain.title}<ArrowRight className="h-4 w-4 text-muted-foreground" /></CardTitle><CardDescription>{domain.description}</CardDescription></CardHeader>
            </Card>
          </Link>
        ))}
      </section>
      ...
      <section aria-label="Resumen canónico de producción" className="grid gap-px overflow-hidden rounded-lg border bg-border sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map((metric) => { const Icon = metric.icon; return <div key={metric.label} className="bg-card px-5 py-5"><div className="flex items-center justify-between gap-3"><p className="text-xs font-medium uppercase tracking-[0.08em] text-muted-foreground">{metric.label}</p><Icon className="h-4 w-4 text-muted-foreground" /></div><p className="mt-4 text-2xl font-semibold tracking-[-0.03em]">{isLoading ? '—' : metric.value}</p><p className="mt-1 text-xs text-muted-foreground">{metric.detail}</p></div>; })}
      </section>

      {!error ? <div className="grid gap-6 xl:grid-cols-3"><Card className="xl:col-span-2"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Database className="h-4 w-4" />Estado de la fuente</CardTitle><CardDescription>Control de cobertura, latencia y procedencia antes de interpretar desempeño.</CardDescription></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2"><div className="rounded-md bg-muted/40 p-4"><p className="text-xs uppercase tracking-[0.08em] text-muted-foreground">Lotes fuente registrados</p><p className="mt-2 text-xl font-semibold">{data?.batches.length ?? 0}</p><p className="mt-1 text-xs text-muted-foreground">Transporte y planta/leyes con período y versión de normalización.</p></div><div className="rounded-md bg-muted/40 p-4"><p className="text-xs uppercase tracking-[0.08em] text-muted-foreground">Latencia transporte → planta</p><p className="mt-2 text-xl font-semibold">{freshnessGap == null ? 'Sin dato' : `${freshnessGap} día${freshnessGap === 1 ? '' : 's'}`}</p><p className="mt-1 text-xs text-muted-foreground">Una diferencia de fecha se interpreta como latencia de evidencia, no como falla productiva.</p></div><div className="rounded-md bg-muted/40 p-4 sm:col-span-2"><p className="text-xs uppercase tracking-[0.08em] text-muted-foreground">Cobertura metalúrgica</p><p className="mt-2 text-sm text-foreground">Los {counts.plantShifts.toLocaleString('es-CL')} turnos de planta están representados en la vista determinística. Los turnos sin ensayo permanecen explícitamente sin ensayo; no se convierten a cero.</p></div></CardContent></Card><Card><CardHeader><CardTitle className="text-base">Despachos de concentrado</CardTitle><CardDescription>{data?.dispatch.status === 'available' ? 'Registros canónicos disponibles.' : 'Pendiente de conciliación histórica.'}</CardDescription></CardHeader><CardContent><p className="text-3xl font-semibold">{isLoading ? '—' : counts.concentrateShipments.toLocaleString('es-CL')}</p><p className="mt-2 text-xs text-muted-foreground">{data?.dispatch.note ?? 'La interfaz no mostrará despachos simulados.'}</p></CardContent></Card></div> : null}
    </div>
  );
}
