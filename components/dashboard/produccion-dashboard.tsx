...
import { Activity, AlertTriangle, ArrowRight, Factory, PackageCheck, Truck, Upload } from 'lucide-react';
...
type CanonicalOverview = {
  ...
  counts: { materialMovements: number; plantShifts: number; metallurgyResults: number; metallurgyAssayed: number; metallurgyPartial: number; metallurgyNoAssay: number; concentrateShipments: number; concentrateAllocations: number; reconciliationPending: number };
  freshness: { latestMaterialMovementDate: string | null; latestPlantOperationDate: string | null; latestShipmentDate: string | null };
...
function formatTons(value: number) {
  return `${value.toLocaleString('es-CL', { maximumFractionDigits: 2 })} t`;
}
...
export function ProduccionDashboard() {
  ...
  const counts = data?.counts ?? { materialMovements: 0, plantShifts: 0, metallurgyResults: 0, metallurgyAssayed: 0, metallurgyPartial: 0, metallurgyNoAssay: 0, concentrateShipments: 0, concentrateAllocations: 0, reconciliationPending: 0 };
  ...
  return <div className="space-y-6">
    ...
    {!error && data ? <section aria-label="Estado de despachos" className="grid gap-px overflow-hidden rounded-lg border bg-border md:grid-cols-4">
      <div className="bg-card px-4 py-3"><p className="text-xs text-muted-foreground">Despachos</p><p className="mt-1 font-medium">{counts.concentrateShipments.toLocaleString('es-CL')}</p></div>
      <div className="bg-card px-4 py-3"><p className="text-xs text-muted-foreground">Toneladas húmedas</p><p className="mt-1 font-medium">{formatTons(data.dispatch.wetMetricTons)}</p></div>
      <div className="bg-card px-4 py-3"><p className="text-xs text-muted-foreground">Allocations</p><p className="mt-1 font-medium">{counts.concentrateAllocations.toLocaleString('es-CL')}</p></div>
      <div className="bg-card px-4 py-3"><p className="text-xs text-muted-foreground">Cobertura de linaje</p><p className="mt-1 font-medium">{data.dispatch.allocationCoveragePct.toLocaleString('es-CL', { maximumFractionDigits: 1 })}%</p></div>
    </section> : null}
