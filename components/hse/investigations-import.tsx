...
type InvestigationsImportProps = {
  onSuccess?: () => void;
};

export function InvestigationsImport({ onSuccess }: InvestigationsImportProps) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch('/api/hse/investigations/import', {
        method: 'POST',
        credentials: 'include',
        body: formData,
      });
      ...
      if (!response.ok) {
        setResult({
          success: false,
          message: 'No se pudieron importar las investigaciones',
      ...
      setResult({
        success: true,
        message: payload.message || 'Investigaciones importadas correctamente',
    ...
    <Card className="border-[var(--secondary)]/25 bg-[var(--secondary)]/5">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          ...
          Importar investigaciones desde Excel
        </CardTitle>
      </CardHeader>
