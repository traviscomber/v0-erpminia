...
export function UsersImportXls({ onImportComplete }: { onImportComplete?: () => void }) {
  ...
  const uploadFile = async (file: File) => {
    ...
    if (!valid) {
      setResult({ success: false, message: 'Solo aceptamos CSV, XLS o XLSX', error: 'Formato no válido' });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        setResult({
          success: false,
          message: 'No se pudieron importar los usuarios',
      ...
      setResult({
        success: true,
        message: payload.message || 'Usuarios importados correctamente',
        ...
        <Alert>
          ...
          <AlertDescription>
            ...
            <div className="rounded bg-muted p-2 font-mono text-sm">
              EMAIL | FULL_NAME | ROLE (superadmin|admin|manager|technician|warehouse_staff|finance_officer|viewer)
            </div>
          </AlertDescription>
        </Alert>
