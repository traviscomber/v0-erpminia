...
type PipelineRow = { order_id?: string | null; order_number?: string | null; order_status?: string | null; order_total?: number | null; supplier_name?: string | null; work_order_number?: string | null; work_order_title?: string | null; quantity_ordered?: number | null; quantity_received?: number | null };
...
type MatchSummary = { invoice_id: string; invoice_number: string; invoice_date: string; order_id: string; order_number: string; net_amount: number; tax_amount: number; total_amount: number; line_count: number; matched_line_count: number; pending_receipt_line_count: number; exception_line_count: number; match_status: string };
type MatchLine = { invoice_id: string; invoice_line_id: string; order_line_id: string; product_code?: string | null; description?: string | null; quantity_ordered: number; quantity_accepted: number; quantity_invoiced: number; ordered_unit_cost: number; invoiced_unit_cost: number; line_match_status: string };
...
function lineStatusLabel(status: string) {
  const labels: Record<string, string> = { matched: 'Coincide', pending_receipt: 'Sin recepción aceptada', quantity_over_receipt: 'Factura supera recepción', quantity_over_order: 'Factura supera OC', price_mismatch: 'Precio distinto', product_mismatch: 'Producto distinto' };
...
export function ProgressiveInvoiceWorkflow() {
  ...
  const invoiceOrderIds = useMemo(() => new Set(summaries.map((row) => row.order_id)), [summaries]);
  const ordersWithoutInvoice = orders.filter((row) => row.order_id && !invoiceOrderIds.has(row.order_id));
  ...
  const openInvoice = (order: PipelineRow) => {
    setSelectedOrder(order); setInvoiceNumber(''); setInvoiceDate(new Date().toISOString().slice(0, 10)); setTaxAmount(''); setDocumentUrl(''); setActionError(null);
    setDraftLines(orderLines.filter((line) => line.order_id === order.order_id).map((line) => ({ orderLineId: line.id, quantity: String(Math.max(0, Number(line.quantity_received || 0))), unitCost: String(Number(line.unit_cost || 0)) })));
  };
  ...
  if (openExceptions.length && exceptionInvoice) {
    ...
    nextActionDescription = `Factura ${approvableInvoice.invoice_number}: OC, recepción y factura coinciden.`;
  ...
  } else if (ordersWithoutInvoice.length) {
    nextActionLabel = 'Registrar factura';
    nextActionDescription = `${ordersWithoutInvoice[0].order_number || 'OC'} aún no tiene factura registrada en este flujo.`;
    nextActionControl = <Button onClick={() => openInvoice(ordersWithoutInvoice[0])} disabled={!canEdit || busy}><FileText className="mr-2 h-4 w-4" />Registrar factura</Button>;
  ...
  return <div className="space-y-6">
    <section className="border-b border-border/70 pb-6"><p className="text-sm font-medium text-muted-foreground">Abastecimiento · Control de factura</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Factura → match → aprobación → pago</h1><p className="mt-2 max-w-3xl text-sm text-muted-foreground">Una sola decisión principal por vez. El three-way match conserva la trazabilidad entre OC, recepción aceptada y factura.</p></section>
    ...
    <Card className="shadow-none"><CardHeader><CardTitle>Expedientes de factura</CardTitle><CardDescription>El detalle queda disponible sin competir con la acción principal.</CardDescription></CardHeader><CardContent className="space-y-3">
      ...
      {summaries.map((summary) => {
        const meta = statusMeta(summary.match_status); const Icon = meta.icon; const invoice = invoices.find((row) => row.id === summary.invoice_id); const exceptions = invoiceExceptions.filter((row) => row.invoice_id === summary.invoice_id); const open = expandedInvoice === summary.invoice_id; const lines = matchLines.filter((row) => row.invoice_id === summary.invoice_id);
        return <div key={summary.invoice_id} className="rounded-lg border p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="font-medium">Factura {summary.invoice_number}</p><Badge variant="outline" className={meta.className}><Icon className="mr-1 h-3.5 w-3.5" />{meta.label}</Badge>{invoice?.status === 'approved' ? <Badge><ShieldCheck className="mr-1 h-3.5 w-3.5" />Pago aprobado</Badge> : null}</div><p className="mt-1 text-sm text-muted-foreground">{summary.order_number} · {money(summary.total_amount)}</p><p className="mt-1 text-xs text-muted-foreground">{summary.matched_line_count}/{summary.line_count} líneas coinciden · {exceptions.filter((row) => row.status === 'open').length} excepción(es) abiertas</p></div><Button size="sm" variant="outline" onClick={() => setExpandedInvoice(open ? null : summary.invoice_id)}>{open ? 'Ocultar detalle' : 'Ver detalle'}</Button></div>
        {open ? <div className="mt-4 space-y-4 border-t pt-3"><div className="space-y-2">{lines.map((line) => <div key={line.invoice_line_id} className="grid gap-2 rounded-md bg-muted/40 p-3 text-sm sm:grid-cols-[1fr_auto]"><div><p className="font-medium">{line.product_code || line.description || 'Línea'}</p><p className="text-xs text-muted-foreground">OC {Number(line.quantity_ordered)} · recibido {Number(line.quantity_accepted)} · facturado {Number(line.quantity_invoiced)}</p></div><div className="text-left sm:text-right"><Badge variant="outline">{lineStatusLabel(line.line_match_status)}</Badge><p className="mt-1 text-xs text-muted-foreground">{money(line.ordered_unit_cost)} → {money(line.invoiced_unit_cost)}</p></div></div>)}</div>{exceptions.length ? <div className="space-y-2"><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Resolución de excepciones</p>{exceptions.map((exception) => <div key={exception.id} className="rounded-md border p-3 text-sm"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><span className="font-medium">{exceptionLabel(exception.exception_type)}</span><Badge variant="outline">{exception.status}</Badge></div>{exception.resolution_notes ? <p className="mt-2 text-xs">{exception.resolution_notes}</p> : null}</div>{exception.status === 'open' ? <Button size="sm" variant="outline" onClick={() => openResolution(exception, 'accepted')} disabled={!canEdit || busy}>Resolver</Button> : null}</div></div>)}</div> : null}</div> : null}
        </div>;
      })}
    </CardContent></Card>

    {ordersWithoutInvoice.length > 1 ? <Card className="shadow-none"><CardHeader><CardTitle>OC pendientes de factura</CardTitle></CardHeader><CardContent className="space-y-2">{ordersWithoutInvoice.slice(1).map((order) => <div key={order.order_id} className="flex items-center justify-between rounded-md border p-3"><div><p className="text-sm font-medium">{order.order_number}</p><p className="text-xs text-muted-foreground">{order.supplier_name || 'Proveedor'} · recibido {Number(order.quantity_received || 0)} de {Number(order.quantity_ordered || 0)}</p></div><Button size="sm" variant="outline" onClick={() => openInvoice(order)} disabled={!canEdit || busy}>Registrar</Button></div>)}</CardContent></Card> : null}

    <Dialog open={Boolean(selectedOrder)} onOpenChange={(open) => { if (!open) setSelectedOrder(null); }}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>Registrar factura proveedor</DialogTitle><DialogDescription>{selectedOrder?.order_number} · compara automáticamente contra OC y recepción aceptada.</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label>Número de factura</Label><Input value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} /></div><div className="space-y-2"><Label>Fecha</Label><Input type="date" value={invoiceDate} onChange={(e) => setInvoiceDate(e.target.value)} /></div><div className="space-y-2"><Label>Impuesto</Label><Input type="number" min="0" value={taxAmount} onChange={(e) => setTaxAmount(e.target.value)} /></div><div className="space-y-2"><Label>URL documento</Label><Input value={documentUrl} onChange={(e) => setDocumentUrl(e.target.value)} placeholder="Opcional" /></div></div><div className="space-y-3">{selectedLines.map((line) => { const draft = draftLines.find((item) => item.orderLineId === line.id); return <div key={line.id} className="grid gap-3 rounded-lg border p-3 sm:grid-cols-[1fr_120px_140px] sm:items-end"><div><p className="text-sm font-medium">{line.product_code || line.description || 'Producto'}</p><p className="text-xs text-muted-foreground">OC {line.quantity_ordered} · recibido {line.quantity_received} {line.unit || ''}</p></div><div className="space-y-1"><Label className="text-xs">Cantidad</Label><Input type="number" min="0" step="any" value={draft?.quantity || ''} onChange={(e) => setDraftLines((rows) => rows.map((row) => row.orderLineId === line.id ? { ...row, quantity: e.target.value } : row))} /></div><div className="space-y-1"><Label className="text-xs">Precio unitario</Label><Input type="number" min="0" step="any" value={draft?.unitCost || ''} onChange={(e) => setDraftLines((rows) => rows.map((row) => row.orderLineId === line.id ? { ...row, unitCost: e.target.value } : row))} /></div></div>; })}</div><div className="rounded-lg bg-muted/50 p-3 text-sm"><div className="flex justify-between"><span>Neto</span><span>{money(netAmount)}</span></div><div className="mt-1 flex justify-between"><span>Impuesto</span><span>{money(taxAmount)}</span></div><div className="mt-2 flex justify-between border-t pt-2 font-medium"><span>Total</span><span>{money(totalAmount)}</span></div></div><DialogFooter><Button variant="outline" onClick={() => setSelectedOrder(null)}>Cancelar</Button><Button onClick={submitInvoice} disabled={busy || !canEdit}>{busy ? 'Guardando...' : 'Guardar y validar'}</Button></DialogFooter></DialogContent></Dialog>
